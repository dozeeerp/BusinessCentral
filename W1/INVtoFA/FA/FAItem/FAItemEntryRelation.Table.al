...
table 51212 "FA Item Entry Relation"
{
    Caption = 'FA Item Entry Relation';
    ...
    procedure InitFromTrackingSpec(TrackingSpecification: Record "FA Tracking Specification")
    begin
        Init();
    ...
    procedure CopyTrackingFromItemJnlLine(ItemJournalLine: Record "FA Item Journal Line")
    begin
    ...
    procedure TransferFieldsTransShptLine(var TransferShipmentLine: Record "FA Transfer Shipment Line")
    begin
        SetSource(DATABASE::"FA Transfer Shipment Line", 0, TransferShipmentLine."Document No.", TransferShipmentLine."Line No.");
        SetOrderInfo(TransferShipmentLine."Transfer Order No.", TransferShipmentLine."Line No.");
    end;

    procedure TransferFieldsTransRcptLine(var TransferReceiptLine: Record "FA Transfer Receipt Line")
    begin
        SetSource(DATABASE::"FA Transfer Receipt Line", 0, TransferReceiptLine."Document No.", TransferReceiptLine."Line No.");
    ...
    procedure TransferFieldsPostedConHeader(var PostedConversionyHeader: Record "Posted Conversion Header")
    begin
        SetSource(DATABASE::"Posted Conversion Header", 0, PostedConversionyHeader."No.", 0);
        SetOrderInfo(PostedConversionyHeader."Order No.", 0);
    end;
    ...
    procedure TransferFieldsDirectTransLine(var FADirectTransLine: Record "FA Direct Trans. Line")
    begin
        SetSource(DATABASE::"FA Direct Trans. Line", 0, FADirectTransLine."Document No.", FADirectTransLine."Line No.");
        SetOrderInfo(FADirectTransLine."Transfer Order No.", FADirectTransLine."Line No.");
    end;
    ...
    local procedure OnAfterInitFromTrackingSpec(var ItemEntryRelation: Record "FA Item Entry Relation"; TrackingSpecification: Record "FA Tracking Specification")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCopyTrackingFromItemLedgEntry(var ItemEntryRelation: Record "FA Item Entry Relation"; ItemLedgerEntry: Record "FA Item Ledger Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCopyTrackingFromItemJnlLine(var ItemEntryRelation: Record "FA Item Entry Relation"; ItemJnlLine: Record "FA Item Journal Line")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCopyTrackingFromSpec(var ItemEntryRelation: Record "FA Item Entry Relation"; TrackingSpecification: Record "FA Tracking Specification")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetSource(ItemEntryRelation: Record "FA Item Entry Relation"; var SourceType: Integer; var SourceSubtype: Option; var SourceID: Code[20]; var SourceRefNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetSource2(ItemEntryRelation: Record "FA Item Entry Relation"; var SourceBatchName: Code[10]; var SourceProdOrderLine: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetOrderInfo(ItemEntryRelation: Record "FA Item Entry Relation"; var OrderNo: Code[20]; var OrderLineNo: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetSourceFilter(var ItemEntryRelation: Record "FA Item Entry Relation"; SourceType: Integer; SourceSubtype: Integer; SourceID: Code[20]; SourceRefNo: Integer; SourceKey: Boolean)
    begin
    end;
}
