...
table 51213 "FA Item Unit of Measure"
{
    Caption = 'FA Item Unit of Measure';
    LookupPageID = "FA Item Units of Measure";
    ...
    fields
    {
        ...
        field(3; "Qty. per Unit of Measure"; Decimal)
        {
            ...
            trigger OnValidate()
            var
                BaseItemUoM: Record "FA Item Unit of Measure";
            begin
                ...
                FAItem.Get("Item No.");
                if FAItem."Base Unit of Measure" = Code then
                    TestField("Qty. per Unit of Measure", 1)
                else
                    if BaseItemUoM.Get(Rec."Item No.", FAItem."Base Unit of Measure") then
        ...
        field(4; "Qty. Rounding Precision"; Decimal)
        {
            ...
            trigger OnValidate()
            var
                ItemUoM: Record "FA Item Unit of Measure";
            begin
                if xRec."Qty. Rounding Precision" <> "Qty. Rounding Precision" then begin
                    CheckNoEntriesWithUoM();
                    FAItem.Get(Rec."Item No.");
                    ItemUoM.SetFilter("Item No.", Rec."Item No.");
                    ItemUoM.SetFilter(Code, '<>%1', FAItem."Base Unit of Measure");
                    if (ItemUoM.FindSet()) then
                        repeat
                            CheckQtyPerUoMPrecision(ItemUoM, Rec."Qty. Rounding Precision");
                        until (ItemUoM.Next() = 0);
                    Session.LogMessage('0000FAR', StrSubstNo(UoMQtyRoundingPercisionChangedTxt, xRec."Qty. Rounding Precision", "Qty. Rounding Precision", FAItem.SystemId), Verbosity::Normal, DataClassification::SystemMetadata, TelemetryScope::ExtensionPublisher, 'Category', UoMLoggingTelemetryCategoryTxt);
                end;
            end;
        }
        field(721; "Coupled to Dataverse"; Boolean)
        {
            ...
            CalcFormula = exist("CRM Integration Record" where("Integration ID" = field(SystemId), "Table ID" = const(Database::"FA Item Unit of Measure")));
        }
    ...
    begin
        if FAItem."No." <> "Item No." then
            FAItem.Get("Item No.");

        // Weight := "Qty. per Unit of Measure" * FAItem."Net Weight";
    ...
    begin
        if FAItem.Get("Item No.") then
            if FAItem."Base Unit of Measure" = xRec.Code then begin
                ItemLedgEntry.SetCurrentKey("FA Item No.", Open);
                ItemLedgEntry.SetRange("FA Item No.", "Item No.");
                ItemLedgEntry.SetRange(Open, true);
                if not ItemLedgEntry.IsEmpty() then
                    Error(Text001, TableCaption(), xRec.Code, "Item No.", FAItem.FieldCaption("Base Unit of Measure"));
            end;
    end;
    ...
    begin
        if FAItem.Get("Item No.") then begin
            if FAItem."Base Unit of Measure" = xRec.Code then
    ...
    //     end;
    local procedure CheckNoRemQtyConversionHeader()
    var
        ConversionHeader: Record "FA Conversion Header";
    begin
        ConversionHeader.SetRange("FA Item No.", "Item No.");
        ConversionHeader.SetRange("Unit of Measure Code", Code);
        ConversionHeader.SetFilter("Remaining Quantity", '<>%1', 0);
        if not ConversionHeader.IsEmpty then
            Error(
                CannotModifyUnitOfMeasureErr, TableCaption(), xRec.Code, "Item No.",
                ConversionHeader.TableCaption(), ConversionHeader.FieldCaption("Remaining Quantity"));
    end;
    ...
    local procedure CheckQtyPerUoMPrecision(ItemUoM: Record "FA Item Unit of Measure"; BaseRoundingPrecision: Decimal)
    //     var
    //         IsHandled: Boolean;
    begin
        //         IsHandled := false;
    ...
    local procedure OnAfterCalcCubage(var ItemUnitOfMeasure: Record "FA Item Unit of Measure")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCalcWeight(var ItemUnitOfMeasure: Record "FA Item Unit of Measure")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckNoOutstandingQtyTransferLine(ItemUnitOfMeasure: Record "FA Item Unit of Measure"; xItemUnitOfMeasure: Record "FA Item Unit of Measure"; var TransferLine: Record "FA Transfer Line"; var IsHandled: Boolean)
    begin
    end;
