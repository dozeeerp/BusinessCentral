...
table 51215 "FA Transfer Line"
{
    DataClassification = CustomerContent;
    Caption = 'FA Transfer Line';
    ...
    fields
    {
        ...
        {
            Caption = 'Item No.';
            TableRelation = "FA Item" where(Type = const(Inventory),
                                        Blocked = const(false));
            ...
            begin
                ...
                if CurrFieldNo <> 0 then
                    TestStatusOpen();
                "FA Item No." := GetItemNo();
                FATransferLineReserve.VerifyChange(Rec, xRec);
                CalcFields("Reserved Qty. Inbnd. (Base)");
                TestField("Reserved Qty. Inbnd. (Base)", 0);
                // WhseValidateSourceLine.TransLineVerifyChange(Rec, xRec);

                TempFATransferLine := Rec;
                Init();
                SystemId := TempFATransferLine.SystemId;
                "FA Item No." := TempFATransferLine."FA Item No.";
        ...
        field(7; "Qty. to Receive"; Decimal)
        {
            ...
            begin
                ...
                if not FATransHeader."Direct Transfer" and ("Direct Transfer" = xRec."Direct Transfer") then
        ...
        field(10; Status; Enum "Conversion Document Status")
        {
            Caption = 'Status';
            Editable = false;
        }
        ...
        field(13; Description; Text[100])
        {
            ...
            begin
                ...
                if (StrLen(Description) <= MaxStrLen(FAItem."No.")) and ("FA Item No." <> '') then
                    ItemDescriptionIsNo := FAItem.Get(Description);

                if ("FA Item No." <> '') and (not ItemDescriptionIsNo) and (Description <> '') then begin
                    FAItem.SetFilter(Description, '''@' + ConvertStr(Description, '''', '?') + '''');
                    if not FAItem.FindFirst() then
                        exit;
                    if FAItem."No." = "FA Item No." then
                        exit;
                    if ConfirmManagement.GetResponseOrDefault(
                        StrSubstNo(AnotherItemWithSameDescrQst, FAItem."No.", FAItem.Description), true)
                    then
                        Validate("FA Item No.", FAItem."No.");
                    exit;
                end;

                if FAItem.TryGetItemNoOpenCard(ReturnValue, Description, false, true, true) then
                    case ReturnValue of
                        '':
                        ...
                        "FA Item No.":
                            ...
                            CurrFieldNo := FieldNo("FA Item No.");
                            Validate("FA Item No.", CopyStr(ReturnValue, 1, MaxStrLen(FAItem."No.")));
                        end;
                    end;
        ...
        field(23; "Unit of Measure Code"; Code[10])
        {
            ...
            begin
                ...
                GetItem();
                // Validate("Qty. per Unit of Measure", UOMMgt.GetQtyPerUnitOfMeasure(FAItem, "Unit of Measure Code"));
                ...
                // "Qty. Rounding Precision" := UOMMgt.GetQtyRoundingPrecision(FAItem, "Unit of Measure Code");
                // "Qty. Rounding Precision (Base)" := UOMMgt.GetQtyRoundingPrecision(FAItem, FAItem."Base Unit of Measure");
        ...
        field(30; "Variant Code"; Code[10])
        {
            ...
            TableRelation = "FA Item Variant".Code where("Item No." = field("FA Item No."), Blocked = const(false));
        ...
        field(39; "Receipt Date"; Date)
        {
            ...
            begin
                ...
                if "Derived From Line No." = 0 then
                    if DerivedLinesExist(FATransferLine, "Document No.", "Line No.") then
                        FATransferLine.ModifyAll("Receipt Date", "Receipt Date");
            end;
        }
        field(40; "Derived From Line No."; Integer)
        {
            Caption = 'Derived From Line No.';
            TableRelation = "FA Transfer Line"."Line No." where("Document No." = field("Document No."));
        }
        ...
        {
            CalcFormula = sum("FA Reservation Entry".Quantity where("Source ID" = field("Document No."),
                                                                  "Source Ref. No." = field("Line No."),
                                                                  "Source Type" = const(51215),
        ...
        {
            CalcFormula = sum("FA Reservation Entry"."Quantity (Base)" where("Source ID" = field("Document No."),
                                                                           "Source Ref. No." = field("Line No."),
                                                                           "Source Type" = const(51215),
                                                                           "Source Subtype" = const("1"),
                                                                           "Source Prod. Order Line" = field("Derived From Line No."),
        ...
        field(8000; "Transfer-from Customer"; Code[50])
        {
            DataClassification = CustomerContent;
        }
        field(8001; "Transfer-to Customer"; Code[50])
        {
            DataClassification = CustomerContent;
        }
    }
    ...
    keys
    {
        ...
        key(Key2; "Transfer-to Code", Status, "Derived From Line No.", "FA Item No.", "Variant Code", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code", "Receipt Date", "In-Transit Code")
        {
            IncludedFields = "Qty. in Transit (Base)", "Outstanding Qty. (Base)", "Shipment Date";
        }
        key(Key3; "Transfer-from Code", Status, "Derived From Line No.", "FA Item No.", "Variant Code", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code", "Shipment Date", "In-Transit Code")
        {
        }
        key(Key4; "FA Item No.", "Variant Code")
        {
        }
        key(Key5; "Transfer-to Code", "Receipt Date", "FA Item No.", "Variant Code")
        {
        }
        key(Key6; "Transfer-from Code", "Shipment Date", "FA Item No.", "Variant Code")
        {
        }
    }
    ...
    begin
        ...
        FATransLine2.Reset();
        FATransLine2.SetFilter("Document No.", FATransHeader."No.");
        if FATransLine2.FindLast() then
            "Line No." := FATransLine2."Line No." + 10000;
        // end;
        FATransferLineReserve.VerifyQuantity(Rec, xRec);
    end;

    trigger OnModify()
    begin
        if ItemExists(xRec."FA Item No.") then
            FATransferLineReserve.VerifyChange(Rec, xRec);
    end;
    ...
    var
        ...
        Reservation: Page "FA Reservation";
    ...
    begin
        ...
        CheckTransferHeader(FATransHeader);
        ...
        "Shipping Time" := FATransHeader."Shipping Time";
        "Outbound Whse. Handling Time" := FATransHeader."Outbound Whse. Handling Time";
        "Inbound Whse. Handling Time" := FATransHeader."Inbound Whse. Handling Time";
        Status := FATransHeader.Status;
        "Direct Transfer" := FATransHeader."Direct Transfer";

        //     OnAfterGetTransHeader(Rec, TransHeader);
    end;

    local procedure CheckTransferHeader(TransferHeader: Record "FA Transfer Header")
    // var
    //     IsHandled: Boolean;
    begin
        //     IsHandled := false;
        ...
        FATransHeader.TestField("Shipment Date");
        FATransHeader.TestField("Receipt Date");
        FATransHeader.TestField("Transfer-from Code");
        FATransHeader.TestField("Transfer-to Code");
        if not FATransHeader."Direct Transfer" and ("Direct Transfer" = xRec."Direct Transfer") then
            FATransHeader.TestField("In-Transit Code");
    end;

    local procedure GetItem()
    begin
        TestField("FA Item No.");
        if "FA Item No." <> FAItem."No." then
    ...
    begin
        ...
        FAItem.TryGetItemNo(ReturnValue, "FA Item No.", true);
        exit(CopyStr(ReturnValue, 1, MaxStrLen("FA Item No.")));
    end;

    procedure BlockDynamicTracking(SetBlock: Boolean)
    begin
        TrackingBlocked := SetBlock;
        FATransferLineReserve.Block(SetBlock);
    end;
    ...
    begin
        ...
        "Dimension Set ID" :=
          DimMgt.GetRecDefaultDimID(
            ...
            "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code", FATransHeader."Dimension Set ID", DATABASE::"FA Item");
        DimMgt.UpdateGlobalDimFromDimSetID("Dimension Set ID", "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code");
    ...
    begin
        ...
        InitNewLine(FATransferLine);
        SelectedItem.SetLoadFields("No.");
        SelectedItem.SetFilter("No.", SelectionFilter);
        if SelectedItem.FindSet() then
            repeat
                AddItem(FATransferLine, SelectedItem."No.");
            until SelectedItem.Next() = 0;
    end;

    procedure AddItem(var FATransferLine: Record "FA Transfer Line"; ItemNo: Code[20])
    begin
        FATransferLine.Init();
        FATransferLine."Line No." += 10000;
        FATransferLine.Validate("FA Item No.", ItemNo);
        FATransferLine.Insert(true);

        // OnAfterAddItem(TransferLine);
    end;

    procedure InitNewLine(var NewTransferLine: Record "FA Transfer Line")
    var
        FATransferLine: Record "FA Transfer Line";
    begin
        NewTransferLine.Copy(Rec);
        FATransferLine.SetLoadFields("Line No.");
        FATransferLine.SetRange("Document No.", NewTransferLine."Document No.");
        if FATransferLine.FindLast() then
            NewTransferLine."Line No." := FATransferLine."Line No."
        else
    ...
    begin
        ...
        GetTransferHeaderNoVerification();
        if FATransHeader."Direct Transfer" then begin
    ...
    begin
        ...
        FATransferLineReserve.CallItemTracking(Rec, Direction);
    end;
    ...
    begin
        ...
        FATransferLineReserve.CallItemTracking(Rec, Direction, true);
    end;
    ...
    begin
        ...
        if FATransHeader."No." <> "Document No." then
            FATransHeader.Get("Document No.");

        // IsHandled := false;
        // OnBeforeTestStatusOpen(Rec, TransHeader, IsHandled);
        // if not IsHandled then
        FATransHeader.TestField(Status, FATransHeader.Status::Open);
    ...
    begin
        Reset();
        SetRange("FA Item No.", OldNo);
        if not Rec.IsEmpty() then
            ModifyAll("FA Item No.", NewNo, true);
    end;
    ...
    begin
        ...
        if Location."Directed Put-away and Pick" then begin
            ...
            if Receive and (Location."Require Receive" or Location."Require Put-away") then begin
                ...
                //      DATABASE::"FA Transfer Line", 1, "Document No.", "Line No.", 0, Quantity)
                // then
                //     ShowDialog := ShowDialog::Error
                // else
            ...
            if not Receive and (Location."Require Shipment" or Location."Require Pick") then begin
                // if WhseValidateSourceLine.WhseLinesExist(
                //      DATABASE::"FA Transfer Line", 0, "Document No.", "Line No.", 0, Quantity)
                // then
                //     ShowDialog := ShowDialog::Error
                // else
    ...
    begin
        ...
        if (FromLocationCode <> '') and ("FA Item No." <> '') then begin
        ...
        if (ToLocationCode <> '') and ("FA Item No." <> '') then begin
    ...
    begin
        exit(StrSubstNo('%1 %2 %3', "Document No.", "Line No.", "FA Item No."));
    end;

    procedure SetReservationEntry(var ReservEntry: Record "FA Reservation Entry"; Direction: Enum "Transfer Direction")
    begin
        ReservEntry.SetSource(
            DATABASE::"FA Transfer Line", Direction.AsInteger(), "Document No.", "Line No.", '', "Derived From Line No.");
        case Direction of
            Direction::Outbound:
                begin
                    ReservEntry.SetItemData(
                        "FA Item No.", Description, "Transfer-from Code", "Variant Code", "Qty. per Unit of Measure");
            ...
            Direction::Inbound:
                begin
                    ReservEntry.SetItemData(
                        "FA Item No.", Description, "Transfer-to Code", "Variant Code", "Qty. per Unit of Measure");
    ...
    procedure SetReservationFilters(var ReservEntry: Record "FA Reservation Entry"; Direction: Enum "Transfer Direction")
    begin
        ReservEntry.SetSourceFilter(DATABASE::"FA Transfer Line", Direction.AsInteger(), "Document No.", "Line No.", false);
    ...
    procedure FilterInboundLinesForReservation(ReservationEntry: Record "FA Reservation Entry"; AvailabilityFilter: Text; Positive: Boolean)
    begin
        Reset();
        SetCurrentKey("Transfer-to Code", "Receipt Date", "FA Item No.", "Variant Code");
    ...
    procedure FilterOutboundLinesForReservation(ReservationEntry: Record "FA Reservation Entry"; AvailabilityFilter: Text; Positive: Boolean)
    begin
        Reset();
        SetCurrentKey("Transfer-from Code", "Shipment Date", "FA Item No.", "Variant Code");
        SetRange("FA Item No.", ReservationEntry."Item No.");
        SetRange("Variant Code", ReservationEntry."Variant Code");
    ...
    procedure ReserveFromInventory(var FATransLine: Record "FA Transfer Line")
    var
    ...
    begin
        if FATransLine.FindSet() then
            repeat
                SourceRecRef.GetTable(FATransLine);
                ReservMgt.SetReservSource(SourceRecRef);
                FATransLine.TestField("Shipment Date");
                FATransLine.CalcFields("Reserved Qty. Outbnd. (Base)");
                ReservMgt.AutoReserveToShip(
                  AutoReserved, '', FATransLine."Shipment Date",
                  FATransLine."Qty. to Ship" - FATransLine."Reserved Quantity Outbnd.",
                  FATransLine."Qty. to Ship (Base)" - FATransLine."Reserved Qty. Outbnd. (Base)");
                if not AutoReserved then
                    Error(CannotAutoReserveErr, FATransLine."Qty. to Ship (Base)", FATransLine."Line No.");
    ...
    var
        ItemLedgEntry: Record "FA Item Ledger Entry";
        TransferLine2: Record "FA Transfer Line";
    begin
    ...
    procedure GetTransferHeader(): Record "FA Transfer Header"
    begin
        if "Document No." <> FATransHeader."No." then
            FATransHeader.Get("Document No.");
    ...
    var
        IEItem: Record "FA Item";
    begin
        exit(IEItem.Get(ItemNo));
    end;

    local procedure DerivedLinesExist(var FATransferLine: Record "FA Transfer Line"; DocumentNo: Code[20]; DerivedFromLineNo: Integer): Boolean
    begin
        FATransferLine.SetRange("Document No.", DocumentNo);
        FATransferLine.SetRange("Derived From Line No.", DerivedFromLineNo);
    ...
    begin
        exit(ItemTrackingMgt.ComposeRowID(DATABASE::"FA Transfer Line", Direction.AsInteger(), "Document No.", '', "Derived From Line No.", "Line No."));
    end;
    ...
    local procedure SetItemLedgerEntryFilters(var ItemLedgEntry: Record "FA Item Ledger Entry")
    begin
    ...
    begin
        DimMgt.AddDimSource(DefaultDimSource, Database::"FA Item", Rec."FA Item No.");
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterShowDimensions(var FATransferLine: Record "FA Transfer Line"; xFATransferLine: Record "FA Transfer Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterShowReservation(var FATransferLine: Record "FA Transfer Line")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeGetReservationQty(var FATransferLine: Record "FA Transfer Line"; var QtyReserved: Decimal; var QtyReservedBase: Decimal; var QtyToReserve: Decimal; var QtyToReserveBase: Decimal; Direction: Integer; var Result: Decimal; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterIsShippedDimChanged(var TransferLine: Record "FA Transfer Line"; var Result: Boolean)
    begin
    end;
