...
codeunit 51211 "FA Item Tracking Management"
{
    Permissions = TableData "FA Item App Entry History" = rd,
                  //TableData "Value Entry Relation" = rd,
                  //TableData "Whse. Item Tracking Line" = rimd,
                  TableData "FA Tracking Specification" = rd;
    //     trigger OnRun()
    //     begin
    ...
    var
        ...
        TempSourceTrackingSpecification: Record "FA Tracking Specification" temporary;
    ...
    procedure SetPointerFilter(var TrackingSpecification: Record "FA Tracking Specification")
    begin
    ...
    procedure CreateTrackingSpecification(var FromReservEntry: Record "FA Reservation Entry"; var ToTrackingSpecification: Record "FA Tracking Specification")
    begin
    ...
    procedure GetItemTrackingSetup(var ItemTrackingCode: Record "Item Tracking Code"; EntryType: Enum "FA Item Ledger Entry Type"; Inbound: Boolean; var ItemTrackingSetup: Record "Item Tracking Setup")
    begin
    ...
    // procedure RetrieveInvoiceSpecification(SourceSpecification: Record "FA Tracking Specification"; var TempInvoicingSpecification: Record "FA Tracking Specification" temporary) OK: Boolean
    // var
    //     TrackingSpecification: Record "FA Tracking Specification";
    //     ReservEntry: Record "FA Reservation Entry";
    //     TempTrackingSpecSummedUp: Record "FA Tracking Specification" temporary;
    //     IsHandled: Boolean;
    // begin
    //     OK := false;
    ...
    // procedure RetrieveInvoiceSpecWithService(SourceSpecification: Record "FA Tracking Specification"; var TempInvoicingSpecification: Record "FA Tracking Specification" temporary; Consume: Boolean) OK: Boolean
    // begin
    //     IsConsume := Consume;
    //     OK := RetrieveInvoiceSpecification(SourceSpecification, TempInvoicingSpecification);
    // end;

    procedure RetrieveItemTracking(FAItemJnlLine: Record "FA Item Journal Line"; var TempHandlingSpecification: Record "FA Tracking Specification" temporary): Boolean
    var
        ReservEntry: Record "FA Reservation Entry";
    begin
        exit(RetrieveItemTrackingFromReservEntry(FAItemJnlLine, ReservEntry, TempHandlingSpecification));
    end;

    procedure RetrieveItemTrackingFromReservEntry(ItemJnlLine: Record "FA Item Journal Line"; var ReservEntry: Record "FA Reservation Entry"; var TempTrackingSpec: Record "FA Tracking Specification" temporary): Boolean
    begin
    ...
    // local procedure RetrieveSubcontrItemTracking(ItemJnlLine: Record "Item Journal Line"; var TempHandlingSpecification: Record "FA Tracking Specification" temporary) Result: Boolean
    // var
    //     ReservEntry: Record "FA Reservation Entry";
    ...
    // procedure RetrieveConsumpItemTracking(ItemJnlLine: Record "Item Journal Line"; var TempHandlingSpecification: Record "FA Tracking Specification" temporary): Boolean
    // var
    //     ReservEntry: Record "FA Reservation Entry";
    // begin
    //     ItemJnlLine.TestField("Order Type", ItemJnlLine."Order Type"::Production);
    ...
    procedure SumUpItemTracking(var ReservEntry: Record "FA Reservation Entry"; var TempHandlingSpecification: Record "FA Tracking Specification" temporary; SumPerLine: Boolean; SumPerTracking: Boolean): Boolean
    var
                ...
                if ReservEntry.TrackingExists() then begin
                    ...
                    if TempHandlingSpecification.FindFirst() then begin
                        ...
                        if ItemTrackingCode."Use Expiration Dates" then begin
                            ItemTrackingSetup.CopyTrackingFromFAReservEntry(ReservEntry);
                            ExpDate :=
    ...
    // procedure SumUpItemTrackingOnlyInventoryOrATO(var ReservationEntry: Record "FA Reservation Entry"; var TrackingSpecification: Record "FA Tracking Specification"; SumPerLine: Boolean; SumPerLotSN: Boolean): Boolean
    // var
    //     TempReservationEntry: Record "FA Reservation Entry" temporary;
    //     QtyBase: Decimal;
    // begin
    ...
    local procedure IsResEntryReservedAgainstInventory(ReservationEntry: Record "FA Reservation Entry"): Boolean
    var
    ...
    begin
        ...
        if ReservationEntry2."Source Type" = Database::"FA Item Ledger Entry" then
            exit(true);

        exit(IsResEntryReservedAgainstATO(ReservationEntry));
    end;

    local procedure IsResEntryReservedAgainstATO(ReservationEntry: Record "FA Reservation Entry"): Boolean
    var
        ReservationEntry2: Record "FA Reservation Entry";
    ...
    begin
        ...
        ReservationEntry2.Get(ReservationEntry."Entry No.", not ReservationEntry.Positive);
        if (ReservationEntry2."Source Type" <> Database::"FA Conversion Header") or
    ...
    local procedure CopyItemTracking3(var ReservEntry: Record "FA Reservation Entry"; ToRowID: Text[250]; SwapSign: Boolean; SkipReservation: Boolean)
    var
    ...
    procedure CollectItemEntryRelation(var TempItemLedgEntry: Record "FA Item Ledger Entry" temporary; SourceType: Integer; SourceSubtype: Integer; SourceID: Code[20]; SourceBatchName: Code[10]; SourceProdOrderLine: Integer; SourceRefNo: Integer; TotalQty: Decimal) Result: Boolean
    var
    ...
    begin
        ItemLedgerEntry.SetCurrentKey("Serial No.", "FA Item No.", Open, "Variant Code", Positive);
        ItemLedgerEntry.SetRange("FA Item No.", ItemNo);
    ...
    procedure SplitWhseJnlLine(TempWhseJnlLine: Record "Warehouse Journal Line" temporary; var TempWhseJnlLine2: Record "Warehouse Journal Line" temporary; var TempWhseSplitTrackingSpec: Record "FA Tracking Specification" temporary; ToTransfer: Boolean)
    var
        ...
        if TempWhseSplitTrackingSpec.FindSet() then
            repeat
                ...
                if ToTransfer then begin
                    ...
                    WhseItemTrackingSetup."Serial No." := TempWhseSplitTrackingSpec."New Serial No.";
                    WhseItemTrackingSetup."Lot No." := TempWhseSplitTrackingSpec."New Lot No.";
                    ...
                    WhseItemTrackingSetup."Serial No." := TempWhseSplitTrackingSpec."Serial No.";
                    WhseItemTrackingSetup."Lot No." := TempWhseSplitTrackingSpec."Lot No.";
                    TempWhseJnlLine2.CopyTrackingFromItemTrackingSetupIfRequired(WhseItemTrackingSetup);
    ...
    // var
    //     ReservEntry: Record "FA Reservation Entry";
    //     OriginalReservEntry: Record "FA Reservation Entry";
    ...
    local procedure RemoveUntrackedSurplus(var ReservationEntry: Record "FA Reservation Entry")
    begin
    ...
    // var
    //     SourceReservEntry: Record "FA Reservation Entry";
    //     WhseManagement: Codeunit "Whse. Management";
    //     SourceType: Integer;
    //     IsHandled: Boolean;
    // begin
    ...
    procedure CreateWhseItemTrkgForResEntry(SourceReservEntry: Record "FA Reservation Entry"; WhseWkshLine: Record "Whse. Worksheet Line")
    var
        WhseItemTrackingLine: Record "Whse. Item Tracking Line";
        WhseManagement: Codeunit "Whse. Management";
        EntryNo: Integer;
    ...
    begin
        ...
        WhseItemTrackingLine."Serial No." := SourceReservEntry."Serial No.";
        WhseItemTrackingLine."Lot No." := SourceReservEntry."Lot No.";
        WhseItemTrackingLine."Warranty Date" := SourceReservEntry."Warranty Date";
        WhseItemTrackingLine."Expiration Date" := SourceReservEntry."Expiration Date";
    ...
    procedure SetGlobalParameters(SourceSpecification2: Record "FA Tracking Specification" temporary; var TempTrackingSpecification2: Record "FA Tracking Specification" temporary; DueDate2: Date)
    begin
    ...
    procedure SynchronizeItemTrackingByPtrs(FromReservEntry: Record "FA Reservation Entry"; ToReservEntry: Record "FA Reservation Entry")
    var
        FromRowID: Text[250];
        ToRowID: Text[250];
    begin
    ...
    procedure SynchronizeItemTracking2(var FromReservEntry: Record "FA Reservation Entry"; ToRowID: Text[250]; DialogText: Text[250])
    var
        ReservEntry2: Record "FA Reservation Entry";
        TempTrkgSpec1: Record "FA Tracking Specification" temporary;
        TempTrkgSpec2: Record "FA Tracking Specification" temporary;
        TempTrkgSpec3: Record "FA Tracking Specification" temporary;
        TempSourceSpec: Record "FA Tracking Specification" temporary;
    ...
    begin
        ...
        if ReservEntry2.IsEmpty() then begin
            ...
            if not TempTrkgSpec3.IsEmpty() then begin
                ...
                if TempTrkgSpec3."Source Type" = Database::"FA Transfer Line" then begin
    ...
    local procedure ModifyTempReservEntrySetIfTransfer(var TempReservEntry: Record "FA Reservation Entry" temporary)
    var
    ...
    begin
        ...
        if TempReservEntry."Source Type" = Database::"FA Transfer Line" then begin
    ...
    //     // procedure SynchronizeWhseItemTracking(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; RegPickNo: Code[20]; Deletion: Boolean)
    //     // var
    //     //     ReservEntry: Record "FA Reservation Entry";
    ...
    procedure FindLastItemLedgerEntry(ItemNo: Code[20]; VariantCode: Code[20]; ItemTrackingSetup: Record "Item Tracking Setup"; var ItemLedgEntry: Record "FA Item Ledger Entry"): Boolean
    var
        IsHandled: Boolean;
        EntryFound: Boolean;
    begin
        ...
        if ItemLedgEntry.GetFilters() <> '' then
            ItemLedgEntry.Reset();
        ItemLedgEntry.SetCurrentKey("FA Item No.", Open, Positive, "Lot No.", "Serial No.");
        ItemLedgEntry.SetRange("FA Item No.", ItemNo);
    ...
    procedure ExistingExpirationDate(TrackingSpecification: Record "FA Tracking Specification"; TestMultiple: Boolean; var EntriesExist: Boolean) ExpiryDate: Date
    var
        ItemTrackingSetup: Record "Item Tracking Setup";
    begin
        ItemTrackingSetup.CopyTrackingFromFATrackingSpec(TrackingSpecification);
        exit(
            ExistingExpirationDate(
                TrackingSpecification."Item No.", TrackingSpecification."Variant Code", ItemTrackingSetup, TestMultiple, EntriesExist));
    end;

    procedure ExistingExpirationDate(ItemLedgerEntry: Record "FA Item Ledger Entry"; TestMultiple: Boolean; var EntriesExist: Boolean) ExpiryDate: Date
    var
        ItemTrackingSetup: Record "Item Tracking Setup";
    begin
        ItemTrackingSetup.CopyTrackingFromFAItemLedgerEntry(ItemLedgerEntry);
        exit(
            ExistingExpirationDate(
                ItemLedgerEntry."FA Item No.", ''//ItemLedgerEntry."Variant Code"
                , ItemTrackingSetup, TestMultiple, EntriesExist));
    end;
    ...
    //     // procedure SumNewLotOnTrackingSpec(var TempTrackingSpecification: Record "FA Tracking Specification" temporary): Decimal
    //     // var
    //     //     TempTrackingSpecification2: Record "FA Tracking Specification";
    //     //     SumLot: Decimal;
    //     // begin
    //     //     SumLot := 0;
    ...
    //     // procedure TestExpDateOnTrackingSpec(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // var
    //     //     IsHandled: Boolean;
    //     // begin
    //     //     IsHandled := false;
    ...
    //     // procedure TestExpDateOnTrackingSpecNew(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // var
    //     //     IsHandled: Boolean;
    //     // begin
    //     //     IsHandled := false;
    ...
    procedure CopyItemLedgEntryTrkgToTransferLine(var ItemLedgEntryBuf: Record "FA Item Ledger Entry"; ToTransferLine: Record "FA Transfer Line")
    var
        ReservEntry: Record "FA Reservation Entry";
        ToReservEntry: Record "FA Reservation Entry";
        QtyBase: Decimal;
        SignFactor: Integer;
        EntriesExist: Boolean;
    begin
    ...
    //     TempReservEntry: Record "FA Reservation Entry" temporary;
    //     ReservEntry: Record "FA Reservation Entry";
    //     xReservEntry: Record "FA Reservation Entry";
    //     ReservEntryBindingCheck: Record "FA Reservation Entry";
    ...
    begin
        ...
        //             IsTransferReceipt := ("Source Type" = Database::"FA Transfer Line") and ("Source Subtype" = 1);
    ...
    //     // local procedure CheckItemTrackingBeforeRegisterNewLines(var TempTrackingSpecificationToCheck: Record "FA Tracking Specification" temporary)
    //     // var
    ...
    //     // local procedure RegisterNewItemTrackingLines(var TempTrackingSpec: Record "FA Tracking Specification" temporary; ItemTrackingLinesBlockCommit: Boolean)
    //     // var
    ...
    procedure RetrieveAppliedExpirationDate(var TempItemLedgEntry: Record "FA Item Ledger Entry" temporary)
    var
        ItemLedgEntry: Record "FA Item Ledger Entry";
    ...
    local procedure ItemTrkgQtyPostedOnSource(SourceTrackingSpec: Record "FA Tracking Specification") Qty: Decimal
    var
        TrackingSpecification: Record "FA Tracking Specification";
        ReservEntry: Record "FA Reservation Entry";
    ...
    begin
        ...
        if SourceTrackingSpec."Source Type" = Database::"FA Transfer Line" then begin
    ...
    local procedure UpdateItemTrackingInTransferReceipt(FromReservEntry: Record "FA Reservation Entry")
    var
        ToReservEntry: Record "FA Reservation Entry";
        ToRowID: Text[250];
    begin
        ToRowID := ComposeRowID(
            Database::"FA Transfer Line", 1, FromReservEntry."Source ID",
            FromReservEntry."Source Batch Name", FromReservEntry."Source Prod. Order Line", FromReservEntry."Source Ref. No.");
    ...
    local procedure SynchronizeItemTrkgTransfer(var ReservEntry: Record "FA Reservation Entry")
    var
        FromReservEntry: Record "FA Reservation Entry";
        ToReservEntry: Record "FA Reservation Entry";
        TempToReservEntry: Record "FA Reservation Entry" temporary;
        TempTrackingSpecification: Record "FA Tracking Specification" temporary;
    begin
    ...
    //     // local procedure InsertReservEntryForSalesLine(var ReservEntry: Record "FA Reservation Entry"; ItemLedgEntryBuf: Record "Item Ledger Entry"; SalesLine: Record "Sales Line"; QtyBase: Decimal; AppliedFromItemEntry: Boolean; var EntriesExist: Boolean)
    //     // begin
    //     //     if QtyBase = 0 then
    //     //         exit;
    ...
    local procedure InsertReservEntryToOutboundTransferLine(var ReservEntry: Record "FA Reservation Entry"; ItemLedgEntryBuf: Record "FA Item Ledger Entry"; TransferLine: Record "FA Transfer Line"; QtyBase: Decimal; var EntriesExist: Boolean)
    begin
        ...
        ReservEntry.SetSource(Database::"FA Transfer Line", 0, TransferLine."Document No.", TransferLine."Line No.", '', 0);
    ...
    procedure InsertReservEntryFromTrackingSpec(TrackingSpecification: Record "FA Tracking Specification"; SourceSubtype: Option; SourceID: Code[20]; SourceRefNo: Integer; QtyBase: Decimal)
    var
        ReservEntry: Record "FA Reservation Entry";
    begin
        if QtyBase = 0 then
    ...
    local procedure InitReservEntry(var ReservEntry: Record "FA Reservation Entry"; ItemLedgEntryBuf: Record "FA Item Ledger Entry"; QtyBase: Decimal; Date: Date; var EntriesExist: Boolean)
    begin
        // with ReservEntry do begin
        ReservEntry.Init();
        ReservEntry."Item No." := ItemLedgEntryBuf."FA Item No.";
    ...
    local procedure IsReservedFromTransferShipment(ReservEntry: Record "FA Reservation Entry"): Boolean
    begin
        exit((ReservEntry."Source Type" = Database::"FA Transfer Line") and (ReservEntry."Source Subtype" = 0));
    end;
    ...
    //     // local procedure CheckQtyToInvoiceMatchItemTracking(var TempTrackingSpecSummedUp: Record "FA Tracking Specification" temporary; var TempTrackingSpecNotInvoiced: Record "FA Tracking Specification" temporary; QtyToInvoiceOnDocLine: Decimal; QtyPerUoM: Decimal)
    //     // var
    ...
    // [Scope('OnPrem')]
    procedure CopyExpirationDateForLot(var TrackingSpecification: Record "FA Tracking Specification")
    var
        CurrTrackingSpec: Record "FA Tracking Specification";
    begin
    ...
    // [Scope('OnPrem')]
    procedure UpdateExpirationDateForLot(var TrackingSpecification: Record "FA Tracking Specification")
    var
        CurrTrackingSpec: Record "FA Tracking Specification";
    begin
        // with TrackingSpecification do begin
    ...
    procedure CreateSerialNoInformation(TrackingSpecification: Record "FA Tracking Specification")
    var
        SerialNoInfo: Record "Serial No. Information";
    ...
    procedure CreateLotNoInformation(TrackingSpecification: Record "FA Tracking Specification")
    var
        LotNoInfo: Record "Lot No. Information";
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeTestExpDateOnTrackingSpec(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; var IsHandled: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterCreateTrackingSpecification(var TrackingSpecification: Record "FA Tracking Specification"; ReservationEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterInsertReservEntryForPurchLine(var ReservEntry: Record "FA Reservation Entry"; PurchLine: Record "Purchase Line")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterInsertReservEntryForSalesLine(var ReservEntry: Record "FA Reservation Entry"; SalesLine: Record "Sales Line")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterSynchronizeItemTracking(var ReservationEntry: Record "FA Reservation Entry"; ToRowID: Text[250])
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeCalcQtyToHandleForTrackedQtyOnDocumentLine(var ReservationEntry: Record "FA Reservation Entry"; var IsHandled: Boolean; SourceType: Integer; SourceSubtype: Option; SourceID: Code[20]; SourceRefNo: Integer; var Result: Decimal)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeCreateWhseItemTrkgForResEntry(var WhseItemTrackingLine: Record "Whse. Item Tracking Line"; SourceReservEntry: Record "FA Reservation Entry"; WhseWkshLine: Record "Whse. Worksheet Line")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeFindTempHandlingSpecification(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeInsertReservEntryForPurchLine(var ReservEntry: Record "FA Reservation Entry"; PurchLine: Record "Purchase Line"; ItemLedgerEntry: Record "Item Ledger Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeInsertReservEntryForSalesLine(var ReservEntry: Record "FA Reservation Entry"; SalesLine: Record "Sales Line"; ItemLedgerEntry: Record "Item Ledger Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeRegisterNewItemTrackingLines(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeRetrieveSubcontrItemTracking(ItemJnlLine: Record "Item Journal Line"; var TempHandlingSpecification: Record "FA Tracking Specification" temporary; var Result: Boolean; var IsHandled: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeSumUpItemTracking(var ReservEntry: Record "FA Reservation Entry"; var TempHandlingSpecification: Record "FA Tracking Specification" temporary; var SumPerLine: Boolean; var SumPerTracking: Boolean; var IsHandled: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeTempTrackingSpecSummedUpModify(var TempTrackingSpecSummedUp: Record "FA Tracking Specification" temporary; var TempInvoicingTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeTempWhseJnlLine2Insert(var TempWhseJnlLineTo: Record "Warehouse Journal Line" temporary; TempWhseJnlLineFrom: Record "Warehouse Journal Line" temporary; var TempSplitTrackingSpec: Record "FA Tracking Specification" temporary; TransferTo: Boolean; WhseSNRequired: Boolean; WhseLNRequired: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeTempHandlingSpecificationInsert(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; ReservationEntry: Record "FA Reservation Entry"; var ItemTrackingCode: Record "Item Tracking Code"; var EntriesExist: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeWhseItemTrackingLineInsert(var WhseItemTrackingLine: Record "Whse. Item Tracking Line"; SourceReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnCalcQtyToHandleForTrackedQtyOnDocumentLineOnAfterReservEntrySetFilters(var ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSyncActivItemTrkgOnBeforeInsertTempReservEntry(var TempReservEntry: Record "FA Reservation Entry" temporary; WhseActivLine: Record "Warehouse Activity Line")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSyncActivItemTrkgOnBeforeTempTrackingSpecModify(var TrackingSpecification: Record "FA Tracking Specification"; var ReservationEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeRegisterItemTrackingLinesLoop(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnCopyItemTracking3OnAfterSwapSign(var TempReservEntry: Record "FA Reservation Entry" temporary; ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnCopyItemTracking3OnAfterTempReservEntryInsert(var TempReservEntry: Record "FA Reservation Entry" temporary; ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnCheckQtyToInvoiceMatchItemTrackingOnAfterTempTrackingSpecSummedUpSetFilters(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnCopyItemLedgEntryTrkgToDocLine(var ItemLedgerEntry: Record "Item Ledger Entry"; var ReservationEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnCreateWhseItemTrackingBatchOnBeforeCreateWhseItemTrackingLines(WhseWkshLine: Record "Whse. Worksheet Line"; var SourceReservEntry: Record "FA Reservation Entry"; SourceType: Integer; var IsHandled: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnInitReservEntryOnBeforeCopyTrackingFromItemLedgEntry(var ReservEntry: Record "FA Reservation Entry"; var ItemLedgEntryBuf: Record "Item Ledger Entry"; var EntriesExist: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnInsertReservationEntryFromTrackingSpecOnBeforeInsert(var ReservEntry: Record "FA Reservation Entry"; TrackingSpecification: Record "FA Tracking Specification")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterInitReservEntry(var ReservEntry: Record "FA Reservation Entry"; ItemLedgerEntry: Record "Item Ledger Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterReserveEntryFilter(ItemJournalLine: Record "Item Journal Line"; var ReservationEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRetrieveItemTrackingFromReservEntryFilter(var ReservEntry: Record "FA Reservation Entry"; ItemJournalLine: Record "Item Journal Line")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRetrieveItemTrackingFromReservEntryOnAfterDeleteReservEntries(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; ItemJnlLine: Record "Item Journal Line"; var ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRetrieveSubcontrItemTrackingOnAfterDeleteReservEntries(var TempHandlingSpecification: Record "FA Tracking Specification" temporary; var ReservationEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRetrieveConsumpItemTrackingOnAfterSetFilters(var ReservationEntry: Record "FA Reservation Entry"; ItemJournalLine: Record "Item Journal Line")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterSynchronizeItemTracking2(FromReservEntry: Record "FA Reservation Entry"; ReservEntry2: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeWhseActivItemTrkgOnAfterSetExpirationDate(var WarehouseActivityLine: Record "Warehouse Activity Line"; var ReservationEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSplitWhseJnlLineOnAfterCheckWhseItemTrkgSetup(var TempWhseJnlLine: Record "Warehouse Journal Line" temporary; var TempWhseSplitTrackingSpec: Record "FA Tracking Specification" temporary; var WhseSNRequired: Boolean; var WhseLNRequired: Boolean; var TempWhseJnlLine2: Record "Warehouse Journal Line" temporary; var IsHandled: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSplitWhseJnlLineOnBeforeCheckSerialNo(var TempWhseTrackingSpecification: Record "FA Tracking Specification" temporary; var IsHandled: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSplitWhseJnlLineOnAfterSetFilters(var TempWhseSplitTrackingSpec: Record "FA Tracking Specification" temporary; var TempWhseJnlLine: Record "Warehouse Journal Line" temporary)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeCopyItemTracking3(var ReservEntry: Record "FA Reservation Entry"; ToRowID: Text[250]; SwapSign: Boolean; SkipReservation: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRegisterNewItemTrackingLinesOnBeforeRegisterItemTrackingLines(var TempTrackingSpecification: Record "FA Tracking Specification"; var ItemTrackingLines: Page "Item Tracking Lines")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSplitWhseJnlLineOnAfterCopyTrackingByToTransfer(var TempWhseSplitTrackingSpec: Record "FA Tracking Specification"; var TempWhseJnlLine2: Record "Warehouse Journal Line"; ToTransfer: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterCreateSNInformation(var SerialNoInfo: Record "Serial No. Information"; TrackingSpecification: Record "FA Tracking Specification")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterCreateLotInformation(var LotNoInfo: Record "Lot No. Information"; var TrackingSpecification: Record "FA Tracking Specification")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeWhseActivItemTrkgOnAfterAssignAbsQty(var TempTrackingSpecification: Record "FA Tracking Specification")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeModifyTempReservEntrySetIfTransfer(var TempReservEntry: Record "FA Reservation Entry"; var IsHandled: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRemoveItemTrkgFromReservEntryOnAfterReservEntryLoop(var ReservEntry: Record "FA Reservation Entry"; OriginalReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeSynchronizeWhseItemTracking(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; RegPickNo: Code[20]; Deletion: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeWhseItemTrackingOnAfterTempTrackingSpecificationLoop(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; RegPickNo: Code[20]; Deletion: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeWhseItemTrackingOnBeforeReservEntryModify(var ReservEntry: Record "FA Reservation Entry"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; RegPickNo: Code[20]; Deletion: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeWhseItemTrackingOnAfterZeroQtyToHandleLoop(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeWhseItemTrackingOnAfterUpdateReservEntryForPick(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTracking2OnAfterCalcShouldInsertTrkgSpec(var TempTrkgSpec1: Record "FA Tracking Specification" temporary; var TempTrkgSpec2: Record "FA Tracking Specification" temporary; var TempTrkgSpec3: Record "FA Tracking Specification" temporary; SignFactor1: Integer; SignFactor2: Integer; var ShouldInsertTrkgSpec: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTracking2OnAfterSyncBothTrackingSpec(var TempTrkgSpec3: Record "FA Tracking Specification" temporary; TempTrkgSpec2: Record "FA Tracking Specification" temporary; TempSourceSpec: Record "FA Tracking Specification" temporary; var TempTrkgSpec1: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTracking2OnAfterAssignNewTrackingSpec(var TempTrkgSpec3: Record "FA Tracking Specification" temporary; TempTrkgSpec1: Record "FA Tracking Specification" temporary; TempSourceSpec: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTracking2OnBeforeDeleteSyncedTrackingSpec(var TempTrkgSpec3: Record "FA Tracking Specification" temporary; TempTrkgSpec1: Record "FA Tracking Specification" temporary; TempTrkgSpec2: Record "FA Tracking Specification" temporary; SignFactor1: Integer; SignFactor2: Integer; var LastEntryNo: Integer)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTrkgTransferOnAfterTempToReservEntrySetFilters(var TempToReservEntry: Record "FA Reservation Entry" temporary; TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTrkgTransferOnBeforeToReservEntryModifyAll(var ToReservEntry: Record "FA Reservation Entry"; TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnItemTrackingExistsOnDocumentLineOnBeforeExit(var TrackingSpecification: Record "FA Tracking Specification"; var ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnAfterTestExpDateOnTrackingSpecNew(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnBeforeTestExpDateOnTrackingSpecNew(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; var IsHandled: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSumUpItemTrackingOnBeforeTempHandlingSpecificationModify(var TempHandlingSpecification: Record "FA Tracking Specification" temporary; ReservEntry: Record "FA Reservation Entry")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRetrieveInvoiceSpecificationOnBeforeFindTrackingSpecification(var TempInvoicingSpecification: Record "FA Tracking Specification" temporary; var TempTrackingSpecSummedUp: Record "FA Tracking Specification" temporary; TrackingSpecification: Record "FA Tracking Specification"; SourceSpecification: Record "FA Tracking Specification"; var OK: Boolean; var IsHandled: Boolean)
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(true, false)]
    //     // local procedure OnCopyItemTracking3OnBeforeReservEntry1Insert(var ReservEntry1: Record "FA Reservation Entry")
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRetrieveInvoiceSpecificationOnAfterTrackingSpecificationSetFilters(SourceSpecification: Record "FA Tracking Specification"; var TrackingSpecification: Record "FA Tracking Specification")
    //     // begin
    //     // end;
    ...
    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRegisterNewItemTrackingLinesOnBeforeCannotMatchItemTrackingErr(var empTrackingSpecification: Record "FA Tracking Specification" temporary; var tyToHandleToNewRegister: Decimal; var QtyToHandleInItemTrackin: Decimal; varQtyToHandleOnSourceDocLine: Decimal; var IsHandled: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnRegisterNewItemTrackingLinesOnBeforeCannotMatchItemTrackingError(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; var QtyToHandleToNewRegister: Decimal; var QtyToHandleInItemTracking: Decimal; var QtyToHandleOnSourceDocLine: Decimal; var IsHandled: Boolean)
    //     // begin
    //     // end;

    //     // [IntegrationEvent(false, false)]
    //     // local procedure OnSynchronizeItemTracking2OnBeforeRegisterItemTrackingLines(var ItemTrackingLines: Page "Item Tracking Lines"; var TempSourceSpec: Record "FA Tracking Specification" temporary; var TempTrkgSpec3: Record "FA Tracking Specification" temporary; var FromReservEntry: Record "FA Reservation Entry"; ReservEntry2: Record "FA Reservation Entry")
    //     // begin
    //     // end;
}