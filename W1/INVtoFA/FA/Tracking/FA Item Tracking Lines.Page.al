...
page 51209 "FA Item Tracking Lines"
{
    Caption = 'FA Item Tracking Lines';
    DataCaptionFields = "Item No.", "Variant Code", Description;
    DelayedInsert = true;
    PageType = Worksheet;
    PopulateAllFields = true;
    SourceTable = "FA Tracking Specification";
            ...
            repeater(Control1)
            {
                ...
                field("Serial No."; Rec."Serial No.")
                {
                    ...
                    begin
                        ...
                        if (Rec."Source Type" = Database::"FA Transfer Line") and (CurrentRunMode = CurrentRunMode::Reclass) then
    ...
    var
        TrackingSpec: Record "FA Tracking Specification";
    ...
    var
        TempReservEntry: Record "FA Reservation Entry" temporary;
    ...
    protected var
        ...
        TempItemTrackLineInsert: Record "FA Tracking Specification" temporary;
        TempItemTrackLineModify: Record "FA Tracking Specification" temporary;
        TempItemTrackLineDelete: Record "FA Tracking Specification" temporary;
        TempItemTrackLineReserv: Record "FA Tracking Specification" temporary;
        TotalTrackingSpecification: Record "FA Tracking Specification";
        SourceTrackingSpecification: Record "FA Tracking Specification";
        ItemTrackingDataCollection: Codeunit "FAItemTracking Data Collection";
    ...
    local procedure InitRecForBarcodeScanner(var TrackingSpecification: Record "FA Tracking Specification" temporary)
    var
        TempRec: Record "FA Tracking Specification" temporary;
    begin
        TempRec.TransferFields(Rec);
        TrackingSpecification.Init();
    ...
    var
        ItemLedgerEntryType: Enum "FA Item Ledger Entry Type";
    begin
    ...
    procedure InsertRecord(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    begin
    ...
    var
        TempTrackingSpec: Record "FA Tracking Specification" temporary;
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    procedure SetSourceSpec(TrackingSpecification: Record "FA Tracking Specification"; AvailabilityDate: Date)
    var
    ...
    begin
        ...
        if (TrackingSpecification."Source Type" = Database::"FA Transfer Line") and (CurrentRunMode = CurrentRunMode::Reclass) then
            CurrentEntryStatus := CurrentEntryStatus::Prospect;
...
#pragma warning restore AL0603
        ...
        if TrackingSpecification."Source Type" = Database::"FA Transfer Line" then
        ...
        if not ExcludePostedEntries then
            ...
            if (TrackingSpecification."Source Type" = Database::"FA Conversion Line") or
               (TrackingSpecification."Source Type" = Database::"FA Conversion Header")
            then
                CollectPostedConversionEntries(TrackingSpecification, TempTrackingSpecification);
        ...
        if (TrackingSpecification."Source Type" = Database::"FA Transfer Line") and
    ...
    local procedure SetSourceSpecForTransferReceipt(TrackingSpecification: Record "FA Tracking Specification"; var ReservEntry: Record "FA Reservation Entry"; var TempTrackingSpecification2: Record "FA Tracking Specification" temporary)
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    protected procedure AddReservEntriesToTempRecSet(var ReservEntry: Record "FA Reservation Entry"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; SwapSign: Boolean; Color: Integer)
    begin
        AddReservEntriesToTempRecSet(ReservEntry, TempTrackingSpecification, SwapSign, Color, 0);
    end;

    protected procedure AddReservEntriesToTempRecSet(var ReservEntry: Record "FA Reservation Entry"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; SwapSign: Boolean; Color: Integer; SrcQtyRoundingPrecision: Decimal)
    var
        FromReservEntry: Record "FA Reservation Entry";
        AddTracking: Boolean;
    begin
                ...
                if ReservEntry.TrackingExists() then begin
                    ...
                    if SecondSourceID = Database::"Warehouse Shipment Line" then
                        if FromReservEntry.Get(ReservEntry."Entry No.", not ReservEntry.Positive) then
                            AddTracking := (FromReservEntry."Source Type" = Database::"FA Conversion Header") = IsAssembleToOrder
                        else
    ...
    local procedure AddToGlobalRecordSet(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    var
        ExpDate: Date;
    ...
    local procedure SetWarehouseControls(TrackingSpecification: Record "FA Tracking Specification")
    //     var
    //         IsHandled: Boolean;
    begin
        //         IsHandled := false;
    ...
    local procedure SetFilters(TrackingSpecification: Record "FA Tracking Specification")
    begin
        Rec.FilterGroup := 2;
    ...
    local procedure CheckItemTrackingLine(TrackingLine: Record "FA Tracking Specification")
    // var
    //     IsHandled: Boolean;
    begin
        // IsHandled := false;
    ...
    var
        xTrackingSpec: Record "FA Tracking Specification";
    begin
    ...
    procedure EntriesAreIdentical(var ReservEntry1: Record "FA Reservation Entry"; var ReservEntry2: Record "FA Reservation Entry"; var IdenticalArray: array[2] of Boolean): Boolean
    begin
        IdenticalArray[1] :=
    ...
    local procedure QtyToHandleAndInvoiceChanged(var ReservEntry1: Record "FA Reservation Entry"; var ReservEntry2: Record "FA Reservation Entry") HasChanged: Boolean
    begin
        HasChanged :=
    ...
    local procedure ShouldModifyTrackingSpecification(TrackingSpecification: Record "FA Tracking Specification"; TrackingSpecificationModify: Record "FA Tracking Specification"): Boolean
    var
        IsHandled: Boolean;
        Result: Boolean;
    begin
    ...
    procedure RegisterChange(var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification"; ChangeType: Option Insert,Modify,FullDelete,PartDelete,ModifyAll; ModifySharedFields: Boolean) OK: Boolean
    var
        ReservEntry1: Record "FA Reservation Entry";
        ReservEntry2: Record "FA Reservation Entry";
        SavedOldTrackingSpecification: Record "FA Tracking Specification";
        CreateReservEntry: Codeunit "FA Create Reserv. Entry";
        ReservationMgt: Codeunit "FA Reservation Management";
        QtyToAdd: Decimal;
        IdenticalArray: array[2] of Boolean;
        IsHandled: Boolean;
    begin
    ...
    local procedure ProcessLateBinding(var NewTrackingSpecification: Record "FA Tracking Specification")
    var
    ...
    local procedure ShouldAddQuantityAsBlank(OldTrackingSpecification: Record "FA Tracking Specification"; NewTrackingSpecification: Record "FA Tracking Specification"): Boolean
    var
        IsHandled: Boolean;
        Result: Boolean;
    begin
    ...
    local procedure ModifyFieldsWithinFilter(var ReservEntry1: Record "FA Reservation Entry"; var TrackingSpecification: Record "FA Tracking Specification")
    begin
    ...
    local procedure SetQtyToHandleAndInvoice(TrackingSpecification: Record "FA Tracking Specification")
    var
        ReservEntry1: Record "FA Reservation Entry";
    ...
    local procedure CollectPostedTransferEntries(TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    var
    ...
    begin
        ...
        case TrackingSpecification."Source Subtype" of
            0: // Outbound
                ItemEntryRelation.SetRange("Source Type", Database::"FA Transfer Shipment Line");
            1: // Inbound
                ItemEntryRelation.SetRange("Source Type", Database::"FA Transfer Receipt Line");
        end;
        ...
        if ItemEntryRelation.Find('-') then
            repeat
                ...
                TempTrackingSpecification."Item No." := ItemLedgerEntry."FA Item No.";
    ...
    //     local procedure CollectPostedAssemblyEntries(TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     var
    ...
    local procedure CollectPostedConversionEntries(TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    var
        ItemEntryRelation: Record "FA Item Entry Relation";
        ItemLedgerEntry: Record "FA Item Ledger Entry";
        CurrentQtyBase: Decimal;
    ...
    begin
        // Used for collecting information about posted Assembly Lines from the created Item Ledger Entries.
        if (TrackingSpecification."Source Type" <> Database::"FA Conversion Line") and
           (TrackingSpecification."Source Type" <> Database::"FA Conversion Header")
        then
            exit;
        ...
        ItemEntryRelation.SetRange("Order No.", TrackingSpecification."Source ID");
        ItemEntryRelation.SetRange("Order Line No.", TrackingSpecification."Source Ref. No.");
        if TrackingSpecification."Source Type" = Database::"FA Conversion Line" then
            ItemEntryRelation.SetRange("Source Type", Database::"Posted Conversion Line")
        else
            ItemEntryRelation.SetRange("Source Type", Database::"Posted Conversion Header");
    ...
    // local procedure CollectPostedOutputEntries(TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    // var
    //     ItemLedgerEntry: Record "FA Item Ledger Entry";
    // begin
    ...
    var
        TempSelectedTrackingSpecification: Record "FA Tracking Specification" temporary;
    ...
    procedure RegisterItemTrackingLines(SourceTrackingSpecification: Record "FA Tracking Specification"; AvailabilityDate: Date; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     var
    //         IsHandled: Boolean;
    begin
    ...
    local procedure TrackingAvailable(var TrackingSpecification: Record "FA Tracking Specification"; LookupMode: Enum "Item Tracking Type"): Boolean
    begin
    ...
    //     procedure GetVariables(var TempTrackingSpecInsert2: Record "FA Tracking Specification" temporary; var TempTrackingSpecModify2: Record "FA Tracking Specification" temporary; var TempTrackingSpecDelete2: Record "FA Tracking Specification" temporary; var Item2: Record Item; var UndefinedQtyArray2: array[3] of Decimal; var SourceQuantityArray2: array[5] of Decimal; var CurrentSignFactor2: Integer; var InsertIsBlocked2: Boolean; var DeleteIsBlocked2: Boolean; var BlockCommit2: Boolean)
    //     begin
    ...
    //     procedure SetVariables(var TempTrackingSpecInsert2: Record "FA Tracking Specification" temporary; var TempTrackingSpecModify2: Record "FA Tracking Specification" temporary; var TempTrackingSpecDelete2: Record "FA Tracking Specification" temporary)
    //     begin
    //         TempItemTrackLineInsert.DeleteAll();
    //         TempItemTrackLineInsert.Reset();
    //         TempTrackingSpecInsert2.Reset();
    ...
    local procedure GetHandleSource(TrackingSpecification: Record "FA Tracking Specification"): Boolean
    var
        WhseActivLine: Record "Warehouse Activity Line";
        QtyToHandleColumnIsHidden: Boolean;
    begin
        // with TrackingSpecification do begin
        if (TrackingSpecification."Source Type" = Database::"FA Item Journal Line") and (TrackingSpecification."Source Subtype" = 6) then begin // 6 => Prod.order line directly
        ...
        QtyToHandleColumnIsHidden :=
            (TrackingSpecification."Source Type" in [Database::"FA Item ledger Entry",
                                Database::"FA Item Journal Line"
                      ...
                      ((TrackingSpecification."Source Type" = Database::"FA Conversion Line") and (TrackingSpecification."Source Subtype" = 0));
        // end;
        //         OnAfterGetHandleSource(TrackingSpecification, QtyToHandleColumnIsHidden);
        exit(not QtyToHandleColumnIsHidden);
    end;

    local procedure GetInvoiceSource(TrackingSpecification: Record "FA Tracking Specification"): Boolean
    var
    ...
    procedure GetTrackingSpec(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    begin
        TempTrackingSpecification.DeleteAll();
    ...
    var
        ReservEntry: Record "FA Reservation Entry";
    begin
        if CurrentSourceType = Database::"FA Transfer Line" then
            exit(false);
        ...
        if not ((ReservEntry."Source Type" = Database::"FA Transfer Line") and (ReservEntry."Source Subtype" = 0)) then
            exit(false);
    ...
    local procedure OnAddReservEntriesToTempRecSetOnAfterTempTrackingSpecificationTransferFields(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; var ReservEntry: Record "FA Reservation Entry")
    begin
    end;
    ...
    local procedure FillSourceQuantityArray(TrackingSpecification: Record "FA Tracking Specification")
    // var
    //     IsHandled: Boolean;
    begin
        // IsHandled := false;
    ...
    local procedure QtyToHandleOrInvoiceDifferFromQuantity(ItemTrackingLine: Record "FA Tracking Specification") HasChanged: Boolean
    begin
        HasChanged :=
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterQtyToHandleOrInvoiceDifferFromQuantity(ItemTrackingLine: Record "FA Tracking Specification"; var HasChanged: Boolean)
    begin
    end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterCopyTrackingSpec(var SourceTrackingSpec: Record "FA Tracking Specification"; var DestTrkgSpec: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterCollectPostedOutputEntries(ItemLedgerEntry: Record "Item Ledger Entry"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterClearTrackingSpec(var OldTrkgSpec: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterOnClosePage(var TrackingSpecification: Record "FA Tracking Specification"; CurrentRunMode: Enum "Item Tracking Run Mode"; CurrentSourceType: Integer; CurrentSourceRowID: Text[250]; SecondSourceRowID: Text[250])
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterCreateReservEntryFor(var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification"; var CreateReservEntry: Codeunit "Create Reserv. Entry")
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterMoveFields(var TrkgSpec: Record "FA Tracking Specification"; var ReservEntry: Record "Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterAssignNewTrackingNo(var TrkgSpec: Record "FA Tracking Specification"; xTrkgSpec: Record "FA Tracking Specification"; FieldID: Integer; var SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterGetHandleSource(TrackingSpecification: Record "FA Tracking Specification"; var QtyToHandleColumnIsHidden: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterGetInvoiceSource(TrackingSpecification: Record "FA Tracking Specification"; var QtyToInvoiceColumnIsHidden: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterRegisterItemTrackingLines(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; var CurrTrackingSpecification: Record "FA Tracking Specification"; var AvailabilityDate: Date; var IsCorrection: Boolean)
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetFilters(var TrackingSpecificationRec: Record "FA Tracking Specification"; TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetSourceSpec(var TrackingSpecification: Record "FA Tracking Specification"; var CurrTrackingSpecification: Record "FA Tracking Specification"; var AvailabilityDate: Date; var BlockCommit: Boolean; FunctionsDemandVisible: Boolean; FunctionsSupplyVisible: Boolean; var QtyToHandleBaseEditable: Boolean; var QuantityBaseEditable: Boolean; var InsertIsBlocked: Boolean)
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(true, false)]
    //     local procedure OnAssignLotNoOnAfterInsert(var TrackingSpecification: Record "FA Tracking Specification"; QtyToCreate: Decimal)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAssignPackageNoOnAfterInsert(var TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(true, false)]
    //     local procedure OnAssignSerialNoBatchOnAfterInsert(var TrackingSpecification: Record "FA Tracking Specification"; QtyToCreate: Integer; CreateLotNo: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnCreateCustomizedSNBatchOnAfterRecInsert(var TrackingSpecification: Record "FA Tracking Specification"; QtyToCreate: Integer)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAssignSerialNoOnAfterAssignQtyToCreateInt(var TrackingSpecification: Record "FA Tracking Specification"; var QtyToCreate: Integer)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnCreateCustomizedSNByPageOnAfterCalcQtyToCreate(var TrackingSpecification: Record "FA Tracking Specification"; var QtyToCreate: Decimal)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAssistEditLotNoOnBeforeCurrPageUdate(var TrackingSpecification: Record "FA Tracking Specification"; xTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeAddToGlobalRecordSet(var TrackingSpecification: Record "FA Tracking Specification"; EntriesExist: Boolean; CurrentSignFactor: Integer; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeAssignNewSerialNo(var TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean; var SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeAssignNewCustomizedSerialNo(var TrackingSpecification: Record "FA Tracking Specification"; var CustomizedSN: Code[50]; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeAssignNewLotNo(var TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean; var SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeAssignNewPackageNo(var TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean; var SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeClosePage(var TrackingSpecification: Record "FA Tracking Specification"; var SkipWriteToDatabase: Boolean; CurrentRunMode: Enum "Item Tracking Run Mode"; CurrentSourceType: Integer; var SourceTrackingSpecification: Record "FA Tracking Specification"; var SourceQuantityArray: array[5] of Decimal; var TotalTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeDeleteRecord(var TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeLotNoAssistEdit(var TrackingSpecification: Record "FA Tracking Specification"; xTrackingSpecification: Record "FA Tracking Specification"; CurrentSignFactor: Integer; var MaxQuantity: Decimal; UndefinedQtyArray: array[3] of Decimal; var IsHandled: Boolean; ForBinCode: Code[20]; Inbound: Boolean; CurrentRunMode: Enum "Item Tracking Run Mode"; ItemTrackingDataCollection: Codeunit "Item Tracking Data Collection"; CurrentSourceType: Integer; SourceQuantityArray: array[5] of Decimal; InsertIsBlocked: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeOnInsertRecord(var TrackingSpecification: Record "FA Tracking Specification"; SourceQuantityArray: array[5] of Decimal; var Result: Boolean; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeOnModifyRecord(var TrackingSpecification: Record "FA Tracking Specification"; xTrackingSpecification: Record "FA Tracking Specification"; InsertIsBlocked: Boolean; var Result: Boolean; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeRegisterChange(var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification"; CurrentSignFactor: Integer; FormRunMode: Option ,Reclass,"Combined Ship/Rcpt","Drop Shipment",Transfer; var IsHandled: Boolean; CurrentPageIsOpen: Boolean; ChangeType: Option Insert,Modify,FullDelete,PartDelete,ModifyAll; ModifySharedFields: Boolean; var ResultOK: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(true, false)]
    //     local procedure OnBeforeSerialNoAssistEdit(var TrackingSpecification: Record "FA Tracking Specification"; xTrackingSpecification: Record "FA Tracking Specification"; CurrentSignFactor: Integer; var IsHandled: Boolean; var MaxQuantity: Decimal; UndefinedQtyArray: array[3] of Decimal; ForBinCode: Code[20]; Inbound: Boolean; CurrentRunMode: Enum "Item Tracking Run Mode"; ItemTrackingDataCollection: Codeunit "Item Tracking Data Collection"; CurrentSourceType: Integer; SourceQuantityArray: array[5] of Decimal; InsertIsBlocked: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeSerialNoOnAfterValidate(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; SecondSourceQuantityArray: array[3] of Decimal)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeSetSourceSpec(var TrackingSpecification: Record "FA Tracking Specification"; var ReservationEntry: Record "Reservation Entry"; var ExcludePostedEntries: Boolean)
    //     begin
    //     end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCollectTempTrackingSpecificationInsert(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; ItemLedgerEntry: Record "FA Item Ledger Entry"; var TrackingSpecification: Record "FA Tracking Specification")
    begin
    end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeUpdateTrackingData(var TrackingSpecification: Record "FA Tracking Specification"; xTrackingSpecification: Record "FA Tracking Specification"; var xTempTrackingSpec: Record "FA Tracking Specification" temporary; CurrentSignFactor: Integer; var SourceQuantityArray: array[5] of Decimal; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeUpdateUndefinedQty(var TrackingSpecification: Record "FA Tracking Specification"; var TotalItemTrackingSpecification: Record "FA Tracking Specification"; var UndefinedQtyArray: array[3] of Decimal; var SourceQuantityArray: array[5] of Decimal; var ReturnValue: Boolean; var IsHandled: Boolean; var ProdOrderLineHandling: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeWriteToDatabase(var TrackingSpecification: Record "FA Tracking Specification"; var CurrentPageIsOpen: Boolean; var BlockCommit: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAddReservEntriesToTempRecSetOnBeforeInsert(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; ReservationEntry: Record "Reservation Entry"; SwapSign: Boolean; Color: Integer)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnInsertRecordOnBeforeTempItemTrackLineInsert(var TempTrackingSpecificationInsert: Record "FA Tracking Specification" temporary; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnProcessLateBindingOnBeforeTempItemTrackLineReservInsert(var TempItemTrackLineReserv: Record "FA Tracking Specification"; CurrentSignFactor: Integer)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnAfterCreateReservEntry(var ReservEntry: Record "Reservation Entry"; TrackingSpecification: Record "FA Tracking Specification"; OldTrackingSpecification: Record "FA Tracking Specification"; CurrentRunMode: Enum "Item Tracking Run Mode"; CurrentSourceType: Integer; TempReservEntry: Record "Reservation Entry" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnAfterFullDelete(var ReservEntry: Record "Reservation Entry"; var NewTrackingSpecification: Record "FA Tracking Specification"; var OldTrackingSpecification: Record "FA Tracking Specification"; CurrentPageIsOpen: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnAfterModify(var NewTrackingSpecification: Record "FA Tracking Specification"; var OldTrackingSpecification: Record "FA Tracking Specification"; CurrentPageIsOpen: Boolean; var SavedOldTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnChangeTypeModifyOnBeforeCheckEntriesAreIdentical(var ReservEntry1: Record "Reservation Entry"; var ReservEntry2: Record "Reservation Entry"; var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification"; var IdenticalArray: array[2] of Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSelectEntriesOnAfterTransferFields(var TempTrackingSpec: Record "FA Tracking Specification" temporary; var TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSetQtyToHandleAndInvoiceOnBeforeReservEntryModify(var ReservationEntry: Record "Reservation Entry"; TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSetQtyToHandleAndInvoiceOnBeforeTrackingSpecModify(var TrackingSpecification: Record "FA Tracking Specification"; var TotalTrackingSpecification: Record "FA Tracking Specification"; var ModifyLine: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(true, false)]
    //     local procedure OnSetSourceSpecOnAfterAssignCurrentEntryStatus(var TrackingSpecification: Record "FA Tracking Specification"; var CurrentEntryStatus: Option; ItemTrackingCode: Record "Item Tracking Code"; var InsertIsBlocked: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnWriteToDatabaseOnBeforeRegisterDecrease(var TempTrackingSpecificationModify: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnWriteToDatabaseOnBeforeRegisterInsert(var TempTrackingSpecificationInsert: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterUpdateExpDateEditable(var TrackingSpecification: Record "FA Tracking Specification"; var ExpirationDateEditable: Boolean; var ItemTrackingCode: Record "Item Tracking Code"; var NewExpirationDateEditable: Boolean; CurrentSignFactor: Integer)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnTestTempSpecificationExistsOnAfterSetFilters(var TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterItemTrackingLinesOnAfterReclass(var TrackingSpecification: Record "FA Tracking Specification"; TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterItemTrackingLinesOnBeforeModify(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnChangeTypeInsertOnBeforeInsertReservEntry(var TrackingSpecification: Record "FA Tracking Specification"; var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification"; FormRunMode: Option)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnWriteToDataOnBeforeCommit(var TrackingSpecification: Record "FA Tracking Specification"; var TempReservEntry: Record "Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnQueryClosePageOnBeforeConfirmClosePage(var TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean; CurrentRunMode: Enum "Item Tracking Run Mode"; var Result: Boolean)
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeFillSourceQuantityArray(var SourceQuantityArray: array[5] of Decimal; TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnValidateSerialNoOnBeforeFindLotNo(var TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeSetSourceSpecForTransferReceipt(var TrackingSpecificationRec: Record "FA Tracking Specification"; var ReservEntry: Record "Reservation Entry"; var TrackingSpecification: Record "FA Tracking Specification"; CurrentRunMode: Enum "Item Tracking Run Mode"; var DeleteIsBlocked: Boolean; var IsHandled: Boolean; var TempTrackingSpecification2: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeCheckItemTrackingLine(var TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean; var SourceQuantityArray: array[5] of Decimal)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeAssignLotNo(var TrackingSpecification: Record "FA Tracking Specification"; var TempItemTrackLineInsert: Record "FA Tracking Specification" temporary; SourceQuantityArray: array[5] of Decimal; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeSetWarehouseControls(TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean; var NewTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeSynchronizeWarehouseItemTracking(TrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnBeforeInsert(var NewTrackingSpecification: Record "FA Tracking Specification"; var OldTrackingSpecification: Record "FA Tracking Specification"; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnAfterInsert(var NewTrackingSpecification: Record "FA Tracking Specification"; var OldTrackingSpecification: Record "FA Tracking Specification"; CurrentPageIsOpen: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnAfterPartialDelete(var NewTrackingSpecification: Record "FA Tracking Specification"; var OldTrackingSpecification: Record "FA Tracking Specification"; var ReservationEntry: Record "Reservation Entry"; CurrentPageIsOpen: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterItemTrackingLinesOnAfterFind(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; IsCorrection: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetQtyToHandleAndInvoiceOnBeforeReservEntryModify(var ReservEntry: Record "Reservation Entry"; var TrackingSpecification: Record "FA Tracking Specification"; var TotalTrackingSpecification: Record "FA Tracking Specification"; var ModifyLine: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSetQtyToHandleAndInvoiceOnAfterSetQtyToHandleAndInvoiceOnFirstReservationEntry(var ReservationEntry: Record "Reservation Entry"; var TrackingSpecification: Record "FA Tracking Specification"; var TotalTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeShouldAddQuantityAsBlank(OldTrackingSpecification: Record "FA Tracking Specification"; NewTrackingSpecification: Record "FA Tracking Specification"; var Result: Boolean; var IsHandled: Boolean)
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnAddToGlobalRecordSetOnAfterTrackingSpecificationCalcSums(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeShouldModifyTrackingSpecification(TrackingSpecification: Record "FA Tracking Specification"; TrackingSpecificationModify: Record "FA Tracking Specification"; var Result: Boolean; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnCalculateSumsOnAfterCalcSums(var TrackingSpecification: Record "FA Tracking Specification"; var SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterUpdateUndefinedQtyArray(TotalTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeSetQtyToHandleAndInvoice(var TrackingSpecification: record "FA Tracking Specification"; IsCorrection: Boolean; CurrentSignFactor: Integer; var TotalTrackingSpecification: record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnBeforeAddItemTrackingToTempRecSet(var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: record "FA Tracking Specification"; CurrentSignFactor: Integer; var TempReservEntry: record "Reservation Entry" temporary);
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAddToGlobalRecordSetOnAfterTrackingSpecificationSetCurrentKey(var TempTrackingSpecification: Record "FA Tracking Specification" temporary)
    //     begin
    //     end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnDeleteRecordOnAfterWMSCheckTrackingChange(TrackingSpecification: Record "FA Tracking Specification"; xTrackingSpecification: Record "FA Tracking Specification"; xRecTrackingSpecification: Record "FA Tracking Specification"; DeleteIsBlocked: Boolean; CurrentSignFactor: Integer; LineIsDemand: Boolean; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(true, false)]
    //     local procedure OnSetSourceSpecOnAfterSetCurrentSourceRowID(CurrentRunMode: Enum "Item Tracking Run Mode"; var CurrentSourceRowID: Text[250]; TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnBeforeLotNoOnAfterValidate(var TempTrackingSpecification: Record "FA Tracking Specification" temporary; SecondSourceQuantityArray: array[3] of Decimal)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSetQtyToHandleAndInvoiceOnAfterSetTrackingSpecificationFilters(var TrackingSpecification: Record "FA Tracking Specification"; SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSetQtyToHandleAndInvoiceOnAfterSetReservEntry1Filters(var ReservEntry1: Record "Reservation Entry"; var TrackingSpecification: Record "FA Tracking Specification"; SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterItemTrackingLinesOnBeforeValidateExistingQuantityBase(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; CurrentRunMode: Enum "Item Tracking Run Mode"; var IsHandled: Boolean)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterItemTrackingLinesOnBeforeFind(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; CurrentRunMode: Enum "Item Tracking Run Mode")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterItemTrackingLinesOnBeforeInsert(var TrackingSpecification: Record "FA Tracking Specification"; var TempTrackingSpecification: Record "FA Tracking Specification" temporary; SourceTrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAssignTrackingNoOnAfterCalcQtyToCreate(var TrackingSpecification: Record "FA Tracking Specification"; var SourceTrackingSpecification: Record "FA Tracking Specification"; var TotalTrackingSpecification: Record "FA Tracking Specification"; var QtyToCreate: Decimal; FieldID: Integer)
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnSelectEntriesOnBeforeResetStatus(var TrackingSpecification: Record "FA Tracking Specification")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnRegisterChangeOnBeforeClearExpirationDate(var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification"; CurrentSignFactor: Integer; CurrentRunMode: Enum "Item Tracking Run Mode"; var IsHandled: Boolean)
    //     begin
    //     end;
}