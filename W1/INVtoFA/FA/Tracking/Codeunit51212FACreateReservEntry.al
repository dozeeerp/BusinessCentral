...
codeunit 51212 "FA Create Reserv. Entry"
{
    Permissions = TableData "FA Reservation Entry" = rim;
    ...
    var
        InsertReservEntry: Record "FA Reservation Entry";
        InsertReservEntry2: Record "FA Reservation Entry";
        LastReservEntry: Record "FA Reservation Entry";
    ...
    procedure CreateReservEntryFor(ForType: Option; ForSubtype: Integer; ForID: Code[20]; ForBatchName: Code[10]; ForProdOrderLine: Integer; ForRefNo: Integer; ForQtyPerUOM: Decimal; Quantity: Decimal; QuantityBase: Decimal; ForReservEntry: Record "FA Reservation Entry")
    var
        Sign: Integer;
    begin
    ...
    procedure CreateReservEntryFrom(FromTrackingSpecification: Record "FA Tracking Specification")
    begin
    ...
    procedure CreateReservEntryExtraFields(var OldTrackingSpecification: Record "FA Tracking Specification"; var NewTrackingSpecification: Record "FA Tracking Specification")
    begin
    ...
    procedure SetNewTrackingFromItemJnlLine(ItemJnlLine: Record "FA Item Journal Line")
    begin
    ...
    procedure SetNewTrackingFromNewTrackingSpecification(FATrackingSpecification: Record "FA Tracking Specification")
    begin
        InsertReservEntry."New Serial No." := FATrackingSpecification."New Serial No.";
        InsertReservEntry."New Lot No." := FATrackingSpecification."New Lot No.";
    ...
    procedure CreateRemainingReservEntry(var OldReservEntry: Record "FA Reservation Entry"; RemainingQuantity: Decimal; RemainingQuantityBase: Decimal)
    var
        OldReservEntry2: Record "FA Reservation Entry";
        FromTracingSpecification: Record "FA Tracking Specification";
    begin
    ...
    procedure TransferReservEntry(NewType: Option; NewSubtype: Integer; NewID: Code[20]; NewBatchName: Code[10]; NewProdOrderLine: Integer; NewRefNo: Integer; QtyPerUOM: Decimal; OldReservEntry: Record "FA Reservation Entry"; TransferQty: Decimal): Decimal
    var
        NewReservEntry: Record "FA Reservation Entry";
        ReservEntry: Record "FA Reservation Entry";
        CarriedReservationEntry: Record "FA Reservation Entry";
        Location: Record Location;
        CarriedItemTrackingSetup: Record "Item Tracking Setup";
        ItemTrkgMgt: Codeunit "FA Item Tracking Management";
    ...
    begin
        ...
        if (NewType = Database::"FA Item Journal Line") and (NewSubtype in [1, 3]) or OverruleItemTracking then
            if InsertReservEntry.NewTrackingExists() then begin
                ...
                if (TransferQty >= 0) and (NewReservEntry.Binding = NewReservEntry.Binding::"Order-to-Order") then begin
                    ...
                    CarriedItemTrackingSetup.CopyTrackingFromFAReservEntry(NewReservEntry);
        ...
        if NewReservEntry."Source Type" = Database::"FA Item Ledger Entry" then
        ...
        if DoCreateNewButUnchangedVersion then begin // If sign has swapped due to negative posting
            ...
            if Abs(TransferQty) < Abs(OldReservEntry."Quantity (Base)") then begin
                ...
                if NewReservEntry."Source Type" = Database::"FA Item Ledger Entry" then begin
                    if NewReservEntry.Get(NewReservEntry."Entry No.", not NewReservEntry.Positive) then begin // Get partner-record
                        SetQtyToHandleAndInvoiceForReservationWithoutItemTracking(NewReservEntry, NewReservEntry."Quantity (Base)", NewReservEntry."Quantity (Base)", false);
                        if NewReservEntry."Quantity (Base)" < 0 then
                            NewReservEntry."Expected Receipt Date" := 0D
                    ...
                    if (NewReservEntry."Source Type" = Database::"Sales Line") and
                       (OldReservEntry."Source Type" = Database::"FA Item Journal Line") and
    ...
    procedure SignFactor(var ReservEntry: Record "FA Reservation Entry"): Integer
    var
        Sign: Integer;
    begin
    ...
    local procedure CheckValidity(var ReservEntry: Record "FA Reservation Entry")
    var
        IsError: Boolean;
        IsHandled: Boolean;
    begin
    ...
    local procedure CheckSourceTypeSubtype(var ReservEntry: Record "FA Reservation Entry") IsError: Boolean
    begin
        case ReservEntry."Source Type" of
            // Database::"Sales Line":
            ...
            Database::"FA Conversion Header", database::"FA Conversion Line":
                IsError := false;
            ...
            Database::"FA Item Journal Line":
                ...
                IsError := (ReservEntry."Source Subtype" <> 1) and (ReservEntry."Source Ref. No." <> 0);
        // Database::"Job Journal Line":
    ...
    procedure GetLastEntry(var ReservEntry: Record "FA Reservation Entry")
    begin
        ReservEntry := LastReservEntry;
    end;

    procedure GetLastInsertReservEntry(var ReservEntry: Record "FA Reservation Entry")
    begin
        ReservEntry := InsertReservEntry;
    end;

    local procedure AdjustDateIfItemLedgerEntry(var ReservEntry: Record "FA Reservation Entry")
    begin
    ...
    local procedure SetupSplitReservEntry(var ReservEntry: Record "FA Reservation Entry"; var ReservEntry2: Record "FA Reservation Entry")
    var
    ...
    local procedure InsertTempTrackingSpecification(var TempTrkgSpec: Record "FA Tracking Specification" temporary; ReservEntry: Record "FA Reservation Entry"; NonReleasedQty: Decimal)
    var
        IsHandled: Boolean;
    begin
        TempTrkgSpec.Init();
        ...
        if not IsHandled then
            if (TempTrkgSpec."Source Type" <> Database::"FA Item Ledger Entry") and
    ...
    var
        ...
        TempTrkgSpec4: Record "FA Tracking Specification" temporary;
        LastEntryNo: Integer;
    ...
    local procedure SplitReservEntry(var ReservEntry: Record "FA Reservation Entry"; var ReservEntry2: Record "FA Reservation Entry"; TrackingSpecificationExists: Boolean; var FirstSplit: Boolean) Result: Boolean
    var
    ...
    local procedure CreateWhseItemTrkgLines(ReservEntry: Record "FA Reservation Entry")
    var
    ...
    procedure UpdateItemTrackingAfterPosting(var ReservEntry: Record "FA Reservation Entry")
    var
    ...
    var
        FromReservationEntry: Record "FA Reservation Entry";
        ToReservationEntry: Record "FA Reservation Entry";
        ItemTrackingManagement: Codeunit "FA Item Tracking Management";
    begin
        if FromReservationEntry.Get(ReservationEntryNo, false) then
            if (FromReservationEntry."Source Type" = Database::"FA Transfer Line") and
    ...
    local procedure NeedSynchronizeItemTrackingToOutboundTransfer(ReservationEntry: Record "FA Reservation Entry"): Boolean
    var
    ...
    internal procedure SetQtyToHandleAndInvoiceForReservationWithoutItemTracking(var ReservationEntry: Record "FA Reservation Entry"; QtyToHandleBase: Decimal; QtyToInvoiceBase: Decimal; DoModify: Boolean)
    begin
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnCreateEntryOnBeforeOnBeforeSplitReservEntry(var ReservEntry: Record "FA Reservation Entry"; var ReservEntry2: Record "Reservation Entry")
    // begin
    // end;

    // [IntegrationEvent(false, false)]
    // local procedure OnCreateEntryOnBeforeSurplusCondition(var ReservEntry: Record "FA Reservation Entry"; QtyToHandleAndInvoiceIsSet: Boolean; var InsertReservEntry: Record "FA Reservation Entry")
    // begin
    // end;

    [IntegrationEvent(false, false)]
    local procedure OnCreateReservEntryExtraFields(var InsertReservEntry: Record "FA Reservation Entry"; OldTrackingSpecification: Record "FA Tracking Specification"; NewTrackingSpecification: Record "FA Tracking Specification")
    begin
    end;
