...
page 51220 "FA Reservation"
{
    ...
    begin
        ...
        case SourceRecRef.Number of
            ...
            Database::"FA Conversion Header":
                OnAfterSetConversionHeader(Rec, ReservEntry)
        end;
    end;

    procedure SetReservEntry(ReservEntry2: Record "FA Reservation Entry")
    begin
        ReservEntry := ReservEntry2;
        UpdateReservMgt();
    end;

    procedure FilterReservEntry(var FilterReservEntry: Record "FA Reservation Entry"; ReservEntrySummary: Record "Entry Summary")
    begin
    ...
    local procedure RelatesToSummEntry(var FilterReservEntry: Record "FA Reservation Entry"; ReservEntrySummary: Record "Entry Summary"): Boolean
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    begin
        ...
        PAGE.RunModal(PAGE::"FA Reservation Entries", ReservEntry2);
    ...
    begin
        ...
        if TempTrackingSpecification.Find('-') then begin
            ...
            // if PAGE.RunModal(PAGE::"FA Item Tracking List", TempReservEntry) = ACTION::LookupOK then begin
    ...
    local procedure OnAfterAutoReserve(ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCancelReservationCurrentLine(ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    [IntegrationEvent(TRUE, false)]
    local procedure OnAfterGetQtyPerUOMFromSource(ReservationEntry: Record "FA Reservation Entry"; var QtyPerUOM: Decimal)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterRelatesToSummEntry(FromEntrySummary: Record "Entry Summary"; var FilterReservEntry: Record "FA Reservation Entry"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterReserveFromCurrentLine(ReservEntry: Record "FA Reservation Entry")
    begin
    end;
    ...
    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetSalesLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetReqLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetPurchLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetTransLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetConversionHeader(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetServiceLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetProdOrderLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetProdOrderComponent(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSetItemJnlLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetJobPlanningLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetAssemblyHeader(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetAssemblyLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    //     [IntegrationEvent(false, false)]
    //     local procedure OnAfterSetPlanningComponent(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    //     begin
    //     end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeAutoReserve(ReservEntry: Record "FA Reservation Entry"; var FullAutoReservation: Boolean; QtyToReserve: Decimal; QtyReserved: Decimal; QtyToReserveBase: Decimal; QtyReservedBase: Decimal; var IsHandled: Boolean);
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeGetSerialLotNo(ReservEntry: Record "FA Reservation Entry"; var ItemTrackingQtyToReserve: Decimal; var ItemTrackingQtyToReserveBase: Decimal; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnGetQtyPerUOMFromSourceRecRef(SourceRecRef: RecordRef; var QtyPerUOM: Decimal; var QtyReserved: Decimal; var QtyReservedBase: Decimal; var QtyToReserve: Decimal; var QtyToReserveBase: Decimal; ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnFilterReservEntryOnAfterFilterSource(var ReservationEntry: Record "FA Reservation Entry"; FromEntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry")
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnGetSerialLotNoOnAfterSetTrackingFields(var ReservationEntry: Record "FA Reservation Entry"; TempReservationEntry: Record "FA Reservation Entry" temporary)
    begin
    end;
    ...
    [IntegrationEvent(TRUE, false)]
    local procedure OnUpdateReservMgt(var ReservationEntry: Record "FA Reservation Entry"; var ReservationManagement: Codeunit "FA Reservation Management")
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnBeforeDrillDownReservedThisLine(var EntrySummary: Record "Entry Summary"; var ReservEntry: Record "FA Reservation Entry"; var ReservEntry2: Record "FA Reservation Entry"; var IsHandled: Boolean)
    begin
    end;

    [IntegrationEvent(true, false)]
    local procedure OnBeforeReservedThisLine(var EntrySummary: Record "Entry Summary"; ReservEntry: Record "FA Reservation Entry"; ReservEntry2: Record "FA Reservation Entry"; TempReservSummEntry2: Record "Entry Summary" temporary; var ReservedQuantity: Decimal; var IsHandled: Boolean)
    begin
    end;
}