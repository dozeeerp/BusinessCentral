...
codeunit 51215 "FA Reservation Engine Mgt."
{
    ...
    var
        ...
        Item: Record "FA Item";
        TempSurplusEntry: Record "FA Reservation Entry" temporary;
        TempSortRec1: Record "FA Reservation Entry" temporary;
        TempSortRec2: Record "FA Reservation Entry" temporary;
        TempSortRec3: Record "FA Reservation Entry" temporary;
        TempSortRec4: Record "FA Reservation Entry" temporary;
    ...
    procedure CancelReservation(ReservEntry: Record "FA Reservation Entry")
    var
    ...
    local procedure RevertDateToSourceDate(var ReservEntry: Record "FA Reservation Entry")
    var
    ...
    begin
        case ReservEntry."Source Type" of
            ...
            Database::"FA Transfer Line":
                begin
                    FATransferLine.Get(ReservEntry."Source ID", ReservEntry."Source Ref. No.");
                    if ReservEntry.Positive then
                        ChangeDateFieldOnResEntry(ReservEntry, FATransferLine."Receipt Date", 0D)
                    else
                        ChangeDateFieldOnResEntry(ReservEntry, 0D, FATransferLine."Shipment Date");
                end;
    ...
    local procedure ChangeDateFieldOnResEntry(var ReservEntry: Record "FA Reservation Entry"; ExpectedReceiptDate: Date; ShipmentDate: Date)
    begin
        ReservEntry."Expected Receipt Date" := ExpectedReceiptDate;
        ReservEntry."Shipment Date" := ShipmentDate;
    end;

    procedure CloseReservEntry(ReservEntry: Record "FA Reservation Entry"; ReTrack: Boolean; DeleteAll: Boolean)
    var
        ReservEntry2: Record "FA Reservation Entry";
        SurplusReservEntry: Record "FA Reservation Entry";
        DummyReservEntry: Record "FA Reservation Entry";
        OriginalReservEntry2: Record "FA Reservation Entry";
        TotalQty: Decimal;
        ...
        if ReservEntry."Reservation Status" <> ReservEntry."Reservation Status"::Surplus then begin
            ...
            if (Item."Order Tracking Policy" = Item."Order Tracking Policy"::None) and
               ...
               (((ReservEntry.Binding = ReservEntry.Binding::"Order-to-Order") and ReservEntry2.Positive) or
                (ReservEntry2."Source Type" = Database::"FA Item Ledger Entry") or not ReservEntry2.TrackingExists())
            then
                ReservEntry2.Delete()
            else begin
    ...
    procedure CloseSurplusTrackingEntry(ReservEntry: Record "FA Reservation Entry")
    var
        ReservEntry2: Record "FA Reservation Entry";
    begin
        ReservEntry.Delete();
    ...
    procedure ModifyReservEntry(ReservEntry: Record "FA Reservation Entry"; NewQuantity: Decimal; NewDescription: Text[100]; ModifyReserved: Boolean)
    var
        TotalQty: Decimal;
        IsHandled: Boolean;
    begin
    ...
    procedure CreateForText(ReservEntry: Record "FA Reservation Entry"): Text[80]
    begin
    ...
    procedure CreateFromText(ReservEntry: Record "FA Reservation Entry"): Text[80]
    begin
    ...
    procedure CreateText(ReservEntry: Record "FA Reservation Entry") SourceTypeDesc: Text[80]
    var
        ...
        case ReservEntry."Source Type" of
            ...
            Database::"FA Conversion Header":
                begin
                    SourceType := SourceType::"Assembly Header";
                    exit(
                      StrSubstNo(SourceDoc3Txt, SelectStr(SourceType, SourceTypeText), ReservEntry."Source ID"));
                end;
    ...
    procedure ModifyShipmentDate(var ReservEntry: Record "FA Reservation Entry"; NewShipmentDate: Date)
    var
        ReservEntry2: Record "FA Reservation Entry";
    begin
    ...
    local procedure ModifyActionMessage(ReservEntry: Record "FA Reservation Entry")
    begin
        GetItem(ReservEntry."Item No.");
        if ReservEntry."Reservation Status" = ReservEntry."Reservation Status"::Surplus then begin
    ...
    procedure ModifyExpectedReceiptDate(var ReservEntry: Record "FA Reservation Entry"; NewExpectedReceiptDate: Date)
    var
        ReservEntry2: Record "FA Reservation Entry";
    begin
        ReservEntry2 := ReservEntry;
    ...
    procedure InitFilterAndSortingFor(var FilterReservEntry: Record "FA Reservation Entry"; SetFilters: Boolean)
    begin
        FilterReservEntry.InitSortingAndFilters(SetFilters);
    end;

    procedure InitFilterAndSortingLookupFor(var FilterReservEntry: Record "FA Reservation Entry"; SetFilters: Boolean)
    begin
        FilterReservEntry.InitSortingAndFilters(SetFilters);
    end;

    procedure ModifyUnitOfMeasure(var ReservEntry: Record "FA Reservation Entry"; NewQtyPerUnitOfMeasure: Decimal)
    var
        ReservEntry2: Record "FA Reservation Entry";
    begin
    ...
    procedure ModifyActionMessageDating(var ReservEntry: Record "FA Reservation Entry")
    var
        ReservEntry2: Record "FA Reservation Entry";
    ...
    begin
        ...
        ActionMessageEntry.SetSourceFilterFromFAReservEntry(ReservEntry);
    ...
    procedure AddItemTrackingToTempRecSet(var TempReservEntry: Record "FA Reservation Entry" temporary; var TrackingSpecification: Record "FA Tracking Specification"; QtyToAdd: Decimal; var QtyToAddAsBlank: Decimal; ItemTrackingCode: Record "Item Tracking Code"): Decimal
    begin
    ...
    local procedure ModifyItemTrkgByReservStatus(var TempReservEntry: Record "FA Reservation Entry" temporary; var TrackingSpecification: Record "FA Tracking Specification"; ReservStatus: Enum "Reservation Status"; var QtyToAdd: Decimal; var QtyToAddAsBlank: Decimal; ItemTrackingCode: Record "Item Tracking Code")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    local procedure ModifyItemTrackingOnTempRec(var TempReservEntry: Record "FA Reservation Entry" temporary; var TrackingSpecification: Record "FA Tracking Specification"; QtyToAdd: Decimal; var QtyToAddAsBlank: Decimal; LastEntryNo: Integer; ItemTrackingCode: Record "Item Tracking Code"; EntryMismatch: Boolean; CalledRecursively: Boolean): Decimal
    var
        TempReservEntryCopy: Record "FA Reservation Entry" temporary;
        ReservEntry1: Record "FA Reservation Entry";
        ReservEntry2: Record "FA Reservation Entry";
        TempReservEntry2: Record "FA Reservation Entry" temporary;
    ...
    begin
        ...
        if Abs(TempReservEntry."Quantity (Base)") > Abs(QtyToAdd) then begin // Split entry
            ...
            if EntryMismatch then begin
                ...
                ReservEntry2."Reservation Status" := ReservEntry2."Reservation Status"::Surplus;
                if ReservEntry2."Source Type" = Database::"FA Item Ledger Entry" then begin
            ...
            if EntryMismatch then begin
                ...
                if (ReservEntry1."Source Type" = Database::"FA Item Ledger Entry") and
    ...
    local procedure VerifySurplusRecord(var ReservEntry: Record "FA Reservation Entry"; var QtyToAddAsBlank: Decimal) Modified: Boolean
    begin
    ...
    local procedure UpdateTempSurplusEntry(var ReservEntry: Record "FA Reservation Entry")
    begin
        if ReservEntry."Reservation Status" <> ReservEntry."Reservation Status"::Surplus then
            exit;
    ...
    procedure CollectAffectedSurplusEntries(var TempReservEntry: Record "FA Reservation Entry" temporary): Boolean
    begin
        TempSurplusEntry.Reset();
    ...
    procedure UpdateOrderTracking(var TempReservEntry: Record "FA Reservation Entry" temporary)
    var
        ReservEntry: Record "FA Reservation Entry";
        SurplusEntry: Record "FA Reservation Entry";
        TempProcessedReservationEntry: Record "FA Reservation Entry" temporary;
    ...
    begin
        ...
        while TempReservEntry.FindSet() do begin
            if FirstLoop then begin
                ...
                if Item."Order Tracking Policy" = Item."Order Tracking Policy"::None then begin
                    repeat
                        if (TempReservEntry."Source Type" = Database::"FA Item Ledger Entry") or not TempReservEntry.TrackingExists() then begin
    ...
    procedure UpdateActionMessages(SurplusEntry: Record "FA Reservation Entry")
    var
        DummyReservEntry: Record "FA Reservation Entry";
        ActionMessageEntry: Record "Action Message Entry";
    begin
    ...
    local procedure UpdateSourcePlanned(ReservEntry: Record "FA Reservation Entry")
    var
    // SalesLine: Record "Sales Line";
    begin
    ...
    local procedure ItemTrackingMismatch(ReservEntry: Record "FA Reservation Entry"; NewItemTrackingSetup: Record "Item Tracking Setup"): Boolean
    var
        ReservEntry2: Record "FA Reservation Entry";
        IsMismatch: Boolean;
    begin
    ...
    procedure InitRecordSet(var ReservEntry: Record "FA Reservation Entry"): Boolean
    var
    ...
    procedure InitRecordSet(var ReservEntry: Record "FA Reservation Entry"; CurrItemTrackingSetup: Record "Item Tracking Setup"): Boolean
    var
        IsDemand: Boolean;
    ...
    procedure NEXTRecord(var ReservEntry: Record "FA Reservation Entry"): Integer
    var
        Found: Boolean;
    begin
        // Used for combining sorting of reservation entries with priorities
    ...
    local procedure SetKeyAndFilters(var ReservEntry: Record "FA Reservation Entry")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    var
        ReservEntry: Record "FA Reservation Entry";
        NewReservEntry: Record "FA Reservation Entry";
        W: Dialog;
    ...
    // procedure SplitTrackingConnection(ReservEntry2: Record "FA Reservation Entry"; NewDate: Date)
    // var
    //     ActionMessageEntry: Record "Action Message Entry";
    //     ReservEntry3: Record "FA Reservation Entry";
    //     DummyReservEntry: Record "FA Reservation Entry";
    // begin
    //     ActionMessageEntry.SetCurrentKey("FA Reservation Entry");
    //     ActionMessageEntry.SetRange("FA Reservation Entry", ReservEntry2."Entry No.");
    //     if not ActionMessageEntry.IsEmpty() then
    //         ActionMessageEntry.DeleteAll();
    ...
    local procedure SaveLostReservQty(ReservEntry: Record "FA Reservation Entry")
    begin
    ...
    local procedure SetItemTracking2(TempReservEntry2: Record "FA Reservation Entry"; var TrackingSpecification2: Record "FA Tracking Specification")
    var
    ...
    procedure ResvExistsForTransHeader(var FATransHeader: Record "FA Transfer Header"): Boolean
    var
        ReservEntry: Record "FA Reservation Entry";
    begin
        ReservEntry.InitSortingAndFilters(true);

        ReservEntry.SetRange("Source Type", Database::"FA Transfer Line");
        ReservEntry.SetRange("Source ID", FATransHeader."No.");

        exit(ResvExistsForHeader(ReservEntry));
    end;

    procedure ResvExistsForHeader(var ReservEntry: Record "FA Reservation Entry"): Boolean
    begin
        ReservEntry.SetRange("Source Batch Name", '');
        ReservEntry.SetRange("Source Prod. Order Line", 0);
    ...
    local procedure TransferLineWithItemTracking(ReservEntry: Record "FA Reservation Entry"): Boolean
    begin
        exit((ReservEntry."Source Type" = Database::"FA Transfer Line") and ReservEntry.TrackingExists());
    end;

    local procedure CheckTrackingNoMismatch(ReservEntry: Record "FA Reservation Entry"; TrackingSpecification: Record "FA Tracking Specification"; TrackingSpecification2: Record "FA Tracking Specification"; ItemTrackingCode: Record "Item Tracking Code"): Boolean
    var
        ItemTrackingSetup: Record "Item Tracking Setup";
    begin
        if ReservEntry.Positive then begin
            ItemTrackingSetup.CopyTrackingFromFATrackingSpec(TrackingSpecification2);
            ItemTrackingSetup.CheckFATrackingMismatch(TrackingSpecification, ItemTrackingCode);
        end else begin
            ItemTrackingSetup.CopyTrackingFromFATrackingSpec(TrackingSpecification);
            ItemTrackingSetup.CheckFATrackingMismatch(TrackingSpecification2, ItemTrackingCode);
        end;
