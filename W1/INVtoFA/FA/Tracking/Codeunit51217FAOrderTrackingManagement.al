...
codeunit 51217 FAOrderTrackingManagement
{
    ...
    var
        ...
        ItemLedgEntry3: Record "FA Item Ledger Entry";
        ...
        TempReservEntryList: Record "FA Reservation Entry" temporary;
    ...
    procedure SetConHeader(var CurrentConHeader: Record "FA Conversion Header")
    begin
        ConHeader := CurrentConHeader;

        ReservEntry.InitSortingAndFilters(false);
        ConHeader.SetReservationFilters(ReservEntry);
        CaptionText := ConHeader.GetSourceCaption();

        if CurrentConHeader."Converted Quantity (Base)" <> 0 then begin
            ItemLedgEntry2.SetCurrentKey("Order Type", "Order No.");
            ItemLedgEntry2.SetRange("Order Type", ItemLedgEntry2."Order Type"::Conversion);
            ItemLedgEntry2.SetRange("Order No.", CurrentConHeader."No.");
    ...
    procedure SetItemLedgEntry(var CurrentItemLedgEntry: Record "FA Item Ledger Entry")
    begin
    ...
    procedure DerivePlanningFilter(var FromReservEntry: Record "FA Reservation Entry"; var ToReservEntry: Record "FA Reservation Entry") OK: Boolean
    var
                ...
                if FilterReqLine."Action Message".AsInteger() > FilterReqLine."Action Message"::New.AsInteger() then
                    case FilterReqLine."Ref. Order Type" of
                        ...
                        FilterReqLine."Ref. Order Type"::Transfer:
                            begin
                                ToReservEntry.SetSourceFilter(DATABASE::"FA Transfer Line", 1, FilterReqLine."Ref. Order No.", FilterReqLine."Ref. Line No.", true);
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterSetSoucreRecord(var SourceRecordVar: Variant; var ReservationEntry: Record "FA Reservation Entry"; var CaptionText: Text; var ItemLedgerEntry2: Record "FA Item Ledger Entry")
    begin
    end;
