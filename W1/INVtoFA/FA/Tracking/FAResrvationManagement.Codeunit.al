...
codeunit 51216 "FA Reservation Management"
{
    Permissions = TableData "FA Item ledger Entry" = rm,
    ...
    var
        ...
        CalcReservEntry: Record "FA Reservation Entry";
        CalcReservEntry2: Record "FA Reservation Entry";
        CalcItemLedgEntry: Record "FA Item Ledger Entry";
        Item: Record "FA Item";
        ...
        CallTrackingSpecification: Record "FA Tracking Specification";
        ...
        DeleteTransLineWithItemReservQst: Label 'FA Transfer order %1 has item reservation. Do you want to delete it anyway?', Comment = '%1 = Document No.';
    ...
    procedure SetCalcReservEntry(TrackingSpecification: Record "FA Tracking Specification"; var ReservEntry: Record "FA Reservation Entry")
    begin
        // Late Binding
    ...
    procedure SetOrderTrackingSurplusEntries(var TempReservEntry: Record "FA Reservation Entry" temporary)
    begin
        // Late Binding
    ...
    // end;
    local procedure SetSourceForConversionHeader()
    var
        ConversionHeader: Record "Fa Conversion Header";
    begin
        SourceRecRef.SetTable(ConversionHeader);
        ConversionHeader.SetReservationEntry(CalcReservEntry);
        // OnSetAssemblyHeaderOnBeforeUpdateReservation(CalcReservEntry, AssemblyHeader);
        UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * ConversionHeader."Remaining Quantity (Base)") < 0);
    end;
    ...
    begin
        SourceRecRef.SetTable(FATransferLine);
        FATransferLine.SetReservationEntry(CalcReservEntry, Direction);
        OnSetTransLineOnBeforeUpdateReservation(CalcReservEntry, FATransferLine);
        UpdateReservation((CreateReservEntry.SignFactor(CalcReservEntry) * FATransferLine."Outstanding Qty. (Base)") <= 0);
    end;
    ...
    local procedure UpdateItemLedgEntryStats(CalcReservEntry: Record "FA Reservation Entry"; var TempEntrySummary: Record "Entry Summary" temporary; var CalcSumValue: Decimal; HandleItemTracking2: Boolean)
    var
        ReservForm: Page "FA Reservation";
        ...
        if CalcItemLedgEntry.ReadPermission then begin
            ...
            if CalcSumValue <> 0 then
                if (CalcSumValue > 0) = Positive then begin
                    ...
                    TempEntrySummary."Table ID" := Database::"FA Item Ledger Entry";
    ...
    local procedure UpdateItemTrackingLineStats(CalcReservEntry: Record "FA Reservation Entry"; var TempEntrySummary: Record "Entry Summary"; AvailabilityDate: Date)
    var
    ...
    begin
        ...
        ReservEntry.SetFilter("Source Type", '<> %1', Database::"FA Item Ledger Entry");
        ...
        if ReservEntry.FindSet() then
            repeat
                ...
                TempEntrySummary.CopyTrackingFromFAReservEntry(ReservEntry);
                if ReservEntry.FindSet() then
                    repeat
    ...
    begin
        ...
        if CalcReservEntry."Source Type" in [Database::"FA Conversion Line", Database::"FA Conversion Header"] then
            StopReservation := not (CalcReservEntry."Source Subtype" = 0);
            ...
            repeat
                ...
                then begin
                    ...
                    CallTrackingSpecification.InitTrackingSpecification(
                      Database::"FA Item Ledger Entry", 0, '', '', 0, CalcItemLedgEntry."Entry No.",
    ...
    begin
        ...
        if TransLine.Find(Search) then
            repeat
                ...
                CallTrackingSpecification.InitTrackingSpecification(
                  Database::"FA Transfer Line", ReservSummEntryNo - Enum::"Reservation Summary Type"::"Transfer Shipment".AsInteger(),
    ...
    procedure InsertReservationEntries(var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; ReservQty: Decimal; Description: Text[100]; ExpectedDate: Date; QtyThisLine: Decimal; QtyThisLineBase: Decimal; TrackingSpecification: Record "FA Tracking Specification") ReservationCreated: Boolean
    begin
    ...
    procedure CreateReservation(Description: Text[100]; ExpectedDate: Date; Quantity: Decimal; QuantityBase: Decimal; TrackingSpecification: Record "FA Tracking Specification")
    begin
        CalcReservEntry.TestField("Source Type");
    ...
    var
        CalcReservEntry4: Record "FA Reservation Entry";
        ReqLine: Record "Requisition Line";
        TrackingMgt: Codeunit FAOrderTrackingManagement;
    ...
    procedure DeleteReservEntries(DeleteAll: Boolean; DownToQuantity: Decimal; var ReservEntry: Record "FA Reservation Entry")
    var
        CalcReservEntry4: Record "FA Reservation Entry";
        SurplusEntry: Record "FA Reservation Entry";
        DummyEntry: Record "FA Reservation Entry";
    ...
    begin
        ...
        CurrentItemTrackingSetup.CopyTrackingFromFAReservEntry(ReservEntry);
                    ...
                    Enum::"Reservation Status"::Reservation:
                        for Release := Release::"Non-Inventory" to Release::Inventory do begin
                            ...
                            repeat
                                ...
                                if (Release = Release::Inventory) = (CalcReservEntry4."Source Type" = Database::"FA Item Ledger Entry") then
                                    if (Abs(ReservEntry."Quantity (Base)") <= Abs(QtyToRelease)) or DeleteAll then begin
                                        ReservEngineMgt.CloseReservEntry(ReservEntry, false, DeleteAll);
                                        SaveTrackingSpecification(ReservEntry, ReservEntry."Quantity (Base)");
                                        QtyToRelease := QtyToRelease - ReservEntry."Quantity (Base)";
            ...
            case EntryStatus of
                0:
                    begin // Reservation
                        ...
                        ValueArray[18] := Enum::"Reservation Summary Type"::"FA Conversion Order Header".AsInteger();
                        ValueArray[19] := Enum::"Reservation Summary Type"::"FA Converion Order Line".AsInteger();
                        ArrayCounter := 19;
                    end;
                1:
    ...
    local procedure QuantityTracked(var ReservEntry: Record "FA Reservation Entry"): Decimal
    var
        ReservEntry2: Record "FA Reservation Entry";
        QtyTracked: Decimal;
    begin
    ...
    local procedure QuantityTracked2(var ReservEntry: Record "FA Reservation Entry"): Decimal
    var
        ReservEntry2: Record "FA Reservation Entry";
        QtyTracked: Decimal;
    begin
    ...
    begin
        ...
        if CalcReservEntry."Source Type" = Database::"FA Item Journal Line" then
            exit;
    ...
    procedure MatchSurplus(var ReservEntry: Record "FA Reservation Entry"; var SurplusEntry: Record "FA Reservation Entry"; QtyToTrack: Decimal; SearchForSupply: Boolean; var AvailabilityDate: Date; TrackingPolicy: Enum "Order Tracking Policy"): Decimal
    var
        ReservEntry2: Record "FA Reservation Entry";
        Search: Text[1];
        NextStep: Integer;
    ...
    local procedure MakeConnection(var FromReservEntry: Record "FA Reservation Entry"; var ToReservEntry: Record "FA Reservation Entry"; Quantity: Decimal; ReservationStatus: Enum "Reservation Status"; AvailabilityDate: Date; Binding: Enum "Reservation Binding")
    var
    ...
    procedure MakeRoomForReservation(var ReservEntry: Record "FA Reservation Entry")
    var
        ReservEntry2: Record "FA Reservation Entry";
        TotalQuantity: Decimal;
        IsHandled: Boolean;
    ...
    local procedure SaveTrackingSpecification(var ReservEntry: Record "FA Reservation Entry"; QtyReleased: Decimal)
    begin
    ...
    procedure CollectTrackingSpecification(var TargetTrackingSpecification: Record "FA Tracking Specification" temporary): Boolean
    begin
        // Used when creating reservations.
        TempTrackingSpecification.Reset();
    ...
    procedure SourceQuantity(var ReservEntry: Record "FA Reservation Entry"; SetAsCurrent: Boolean): Decimal
    begin
        exit(GetSourceRecordValue(ReservEntry, SetAsCurrent, 0));
    end;

    procedure FilterReservFor(SourceRecRef: RecordRef; var ReservEntry: Record "FA Reservation Entry"; Direction: Enum "Transfer Direction") CaptionText: Text
    begin
    ...
    procedure GetSourceRecordValue(var ReservEntry: Record "FA Reservation Entry"; SetAsCurrent: Boolean; ReturnOption: Option "Net Qty. (Base)","Gross Qty. (Base)") SourceQty: Decimal
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    local procedure GetItemSetup(var ReservEntry: Record "FA Reservation Entry")
    var
    ...
    procedure MarkReservConnection(var ReservEntry: Record "FA Reservation Entry"; TargetReservEntry: Record "FA Reservation Entry") ReservedQuantity: Decimal
    var
        ReservEntry2: Record "FA Reservation Entry";
        SignFactor: Integer;
        IsHandled: Boolean;
    ...
    procedure IssueActionMessage(var SurplusEntry: Record "FA Reservation Entry"; UseGlobalSettings: Boolean; AllDeletedEntry: Record "FA Reservation Entry")
    var
        ReservEntry: Record "FA Reservation Entry";
        ReservEntry2: Record "FA Reservation Entry";
        ReservEntry3: Record "FA Reservation Entry";
        ActionMessageEntry: Record "Action Message Entry";
    ...
    begin
        ...
        if SurplusEntry."Quantity (Base)" > 0 then begin // Supply: Issue AM directly
            ...
            ActionMessageEntry.TransferFromFAReservEntry(SurplusEntry);
            ...
            if Found then begin
                ActionMessageEntry.TransferFromFAReservEntry(ReservEntry);
                ...
                //         ActionMessageEntry."Source Type" := Database::"FA Transfer Line";
    ...
    procedure FindDate(var ReservEntry: Record "FA Reservation Entry"; Which: Option "Earliest Shipment","Latest Receipt"; ReturnRecord: Boolean): Date
    var
        ReservEntry2: Record "FA Reservation Entry";
        LastDate: Date;
    begin
    ...
    var
        FilterReservEntry: Record "FA Reservation Entry";
        ReservEntry2: Record "FA Reservation Entry";
        ReqLine: Record "Requisition Line";
    begin
    ...
    begin
        ActionMessageEntry.Reset();
        ActionMessageEntry.FilterFromFAReservEntry(CalcReservEntry);
    ...
    local procedure ItemTrackingExist(var ReservEntry: Record "FA Reservation Entry"): Boolean
    var
        ReservEntry2: Record "FA Reservation Entry";
    begin
    ...
    procedure SetTrackingFromReservEntry(ReservEntry: Record "FA Reservation Entry")
    begin
    ...
    procedure SetMatchFilter(var ReservEntry: Record "FA Reservation Entry"; var FilterReservEntry: Record "FA Reservation Entry"; SearchForSupply: Boolean; AvailabilityDate: Date)
    begin
    ...
    local procedure CalcAvailAllocQuantities(
        Item: Record "FA Item"; WhseActivLine: Record "Warehouse Activity Line";
    ...
    local procedure SaveItemTrackingAsSurplus(var ReservEntry: Record "FA Reservation Entry"; NewQty: Decimal; NewQtyBase: Decimal) QuantityIsValidated: Boolean
    var
        SurplusEntry: Record "FA Reservation Entry";
        CreateReservEntry2: Codeunit "FA Create Reserv. Entry";
        QtyToSave: Decimal;
    ...
    begin
        ...
        if ReservEntry."Source Type" = Database::"FA Item Ledger Entry" then
            exit;
    ...
    local procedure ProdJnlLineEntry(ReservationEntry: Record "FA Reservation Entry"): Boolean
    begin
        exit((ReservationEntry."Source Type" = Database::"FA Item Journal Line") and (ReservationEntry."Source Subtype" = 6));
    end;

    local procedure CalcDownToQtySyncingToAssembly(ReservEntry: Record "FA Reservation Entry"): Decimal
    var
    ...
    local procedure CalcCurrLineReservQtyOnPicksShips(ReservationEntry: Record "FA Reservation Entry"): Decimal
    var
        ReservEntry: Record "FA Reservation Entry";
    ...
    local procedure CheckQuantityIsCompletelyReleased(QtyToRelease: Decimal; DeleteAll: Boolean; CurrentItemTrackingSetup: Record "Item Tracking Setup"; ReservEntry: Record "FA Reservation Entry")
    var
        IsHandled: Boolean;
    begin
        IsHandled := false;
    ...
    var
        ReservationEntryPositive: Record "FA Reservation Entry";
    begin
        if ReservationEntryPositive.Get(ReservationEntryNo, true) then
            exit(ReservationEntryPositive."Source Type" = Database::"FA Item Ledger Entry");
    ...
    procedure SetQtyToReserveDownToTrackedQuantity(ReservEntry: Record "FA Reservation Entry"; RowID: Text[250]; var QtyThisLine: Decimal; var QtyThisLineBase: Decimal)
    var
        FilterReservEntry: Record "FA Reservation Entry";
    ...
    local procedure IsSpecialOrderOrDropShipment(ReservationEntry: Record "FA Reservation Entry"): Boolean
    var
        SalesLine: Record "Sales Line";
    ...
    procedure FindUnfinishedSpecialOrderSalesNo(ItemLedgerEntry: Record "FA Item Ledger Entry") Result: Code[20]
    var
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterCalcReservation(var ReservEntry: Record "FA Reservation Entry"; var ItemLedgEntry: Record "FA Item Ledger Entry"; var ResSummEntryNo: Integer; var QtyThisLine: Decimal; var QtyThisLineBase: Decimal; TotalAvailQty: Decimal)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterAutoReserveItemLedgEntry(var CalcItemLedgEntry: Record "FA Item Ledger Entry"; var RemainingQtyToReserveBase: Decimal)
    begin
    end;

    // [IntegrationEvent(false, false)]
    // local procedure OnAfterGetAvailabilityFilter2(var ReservationEntry: Record "FA Reservation Entry"; AvailabilityDate: Date; SearchForSupply: Boolean; Result: Text[80])
    // begin
    // end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterInsertReservationEntries(var TrackingSpecification: Record "FA Tracking Specification"; var CalcReservEntry: Record "FA Reservation Entry"; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; var QtyThisLine: Decimal; var QtyThisLineBase: Decimal; var ReservationCreated: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnAfterSetReservSource(var SourceRecRef: RecordRef; var CalcReservEntry: Record "FA Reservation Entry"; var Direction: Enum "Transfer Direction")
    begin
    end;
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnAutoReserveItemLedgEntryOnFindFirstItemLedgEntry(CalcReservEntry: Record "Reservation Entry"; var CalcItemLedgEntry: Record "FA Item Ledger Entry"; var InvSearch: Text[1]; var IsHandled: Boolean; var IsFound: Boolean)
    // begin
    // end;
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnAutoReserveOneLineOnAfterUpdateSearchNextStep(var Item: Record "FA Item"; var Positive: Boolean; var Search: Text[1]; var NextStep: Integer; var InvSearch: Text[1]; InvNextStep: Integer; var RemainingQtyToReserve: Decimal)
    // begin
    // end;

    [IntegrationEvent(false, false)]
    local procedure OnAutoReserveItemLedgEntryOnFindNextItemLedgEntry(CalcReservEntry: Record "FA Reservation Entry"; var CalcItemLedgEntry: Record "FA Item Ledger Entry"; var InvSearch: Text[1]; var IsHandled: Boolean; var IsFound: Boolean)
    begin
    end;

    // [IntegrationEvent(false, false)]
    // local procedure OnAfterSetSourceForSalesLine(var CalcReservEntry: Record "FA Reservation Entry"; SalesLine: Record "Sales Line")
    // begin
    // end;
    ...
    // [IntegrationEvent(true, false)]
    // local procedure OnBeforeAutoReserveItemLedgEntry(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; var IsReserved: Boolean; CalcReservEntry: Record "Reservation Entry"; var CalcItemLedgerEntry: Record "FA Item Ledger Entry"; var ItemTrackingCode: Record "Item Tracking Code"; Positive: Boolean)
    // begin
    // end;
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnBeforeAutoReserveProdOrderLine(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; var IsReserved: Boolean; Search: Text[1]; NextStep: Integer; CalcReservEntry: Record "FA Reservation Entry")
    // begin
    // end;

    // [IntegrationEvent(false, false)]
    // local procedure OnBeforeAutoReserveProdOrderComp(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; var IsReserved: Boolean; Search: Text[1]; NextStep: Integer; CalcReservEntry: Record "FA Reservation Entry")
    // begin
    // end;

    // [IntegrationEvent(false, false)]
    // local procedure OnBeforeAutoReserveAssemblyHeader(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; var IsReserved: Boolean; Search: Text[1]; NextStep: Integer; CalcReservEntry: Record "FA Reservation Entry")
    // begin
    // end;

    // [IntegrationEvent(false, false)]
    // local procedure OnBeforeAutoReserveAssemblyLine(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; var IsReserved: Boolean; Search: Text[1]; NextStep: Integer; CalcReservEntry: Record "FA Reservation Entry")
    // begin
    // end;

    [IntegrationEvent(false, false)]
    local procedure OnBeforeAutoReserveTransLine(ReservSummEntryNo: Integer; var RemainingQtyToReserve: Decimal; var RemainingQtyToReserveBase: Decimal; Description: Text[100]; AvailabilityDate: Date; var IsReserved: Boolean; var Search: Text[1]; var NextStep: Integer; CalcReservEntry: Record "FA Reservation Entry")
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCreateReservation(var TrkgSpec: Record "FA Tracking Specification"; var ReservEntry: Record "FA Reservation Entry"; var ItemLedgEntry: Record "FA Item Ledger Entry")
    begin
    end;
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnBeforeFindUnfinishedSpecialOrderSalesNo(ItemLedgerEntry: Record "FA Item Ledger Entry"; var Result: Code[20]; var IsHandled: Boolean)
    // begin
    // end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnGetSourceRecordValue(var ReservEntry: Record "FA Reservation Entry"; SetAsCurrent: Boolean; ReturnOption: Option "Net Qty. (Base)","Gross Qty. (Base)"; var ReturnQty: Decimal; var SourceRecRef: RecordRef; var IsHandled: Boolean)
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnInsertReservationEntriesOnBeforeCreateReservation(var TrackingSpecification: Record "FA Tracking Specification"; var CalcReservEntry: Record "FA Reservation Entry")
    begin
    end;
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnSetItemJnlLineOnBeforeUpdateReservation(var ReservEntry: Record "Reservation Entry"; ItemJnlLine: Record "FA Item Journal Line")
    // begin
    // end;

    // [IntegrationEvent(false, false)]
    // local procedure OnSetItemLedgEntryOnBeforeUpdateReservation(var ReservEntry: Record "Reservation Entry"; ItemLedgerEntry: Record "FA Item Ledger Entry")
    // begin
    // end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnSetTransLineOnBeforeUpdateReservation(var ReservEntry: Record "FA Reservation Entry"; TransferLine: Record "FA Transfer Line")
    begin
    end;
    ...
    // [IntegrationEvent(false, false)]
    // local procedure OnUpdateItemLedgEntryStatsUpdateTotals(CalcReservEntry: Record "Reservation Entry"; var CalcItemLedgEntry: Record "FA Item Ledger Entry"; TotalAvailQty: Decimal; QtyOnOutBound: Decimal; var CalcSumValue: Decimal; var TempEntrySummary: Record "Entry Summary"; var IsHandled: Boolean)
    // begin
    // end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnUpdateStatistics(CalcReservEntry: Record "FA Reservation Entry"; var ReservSummEntry: Record "Entry Summary"; AvailabilityDate: Date; Positive: Boolean; var TotalQuantity: Decimal; HandleItemTracking2: Boolean; var QtyOnOutBound: Decimal)
    begin
    end;
