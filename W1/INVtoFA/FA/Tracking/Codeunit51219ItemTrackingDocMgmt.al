...
codeunit 51219 "FA Item Tracking Doc. Mgmt"
{
    ...
    local procedure AddTempRecordToSet(var TempItemLedgEntry: Record "FA Item Ledger Entry" temporary; SignFactor: Integer)
    var
        TempItemLedgEntry2: Record "FA Item Ledger Entry" temporary;
    begin
    ...
    // procedure FillTrackingSpecBufferFromILE(var TempItemLedgEntry: Record "FA Item Ledger Entry" temporary; var TempTrackingSpecBuffer: Record "FA Tracking Specification" temporary; Type: Integer; Subtype: Integer; ID: Code[20]; BatchName: Code[10]; ProdOrderLine: Integer; RefNo: Integer; Description: Text[100])
    // var
    ...
    //                 ItemTrackingSetup.CopyTrackingFromFAItemLedgerEntry(TempItemLedgEntry);
    //                 FillTrackingSpecBuffer(
    //                     TempTrackingSpecBuffer, Type, Subtype, ID, BatchName,
    //                     ProdOrderLine, RefNo, Description, TempItemLedgEntry."FA Item No.", TempItemLedgEntry."Variant Code",
    ...
    // procedure FindShptRcptEntries(var TempTrackingSpecBuffer: Record "FA Tracking Specification" temporary; Type: Integer; Subtype: Integer; ID: Code[20]; BatchName: Code[10]; ProdOrderLine: Integer; RefNo: Integer; Description: Text[100])
    // var
    ...
    procedure RetrieveEntriesFromShptRcpt(var TempItemLedgEntry: Record "FA Item Ledger Entry" temporary; Type: Integer; Subtype: Integer; ID: Code[20]; BatchName: Code[10]; ProdOrderLine: Integer; RefNo: Integer)
    var
    ...
    begin
        ...
        if SourceNo <> '' then begin
            ItemLedgEntry.SetCurrentKey("Source Type", "Source No.", "FA Item No.");
        ...
        Window.Close();
        PAGE.RunModal(PAGE::"FA Item Tracking Entries", TempItemLedgEntry);
    end;
    ...
    begin
        RetrieveEntriesFromShptRcpt(TempFAItemLedgEntry, Type, Subtype, ID, BatchName, ProdOrderLine, RefNo);
        if not TempFAItemLedgEntry.IsEmpty() then begin
            PAGE.RunModal(PAGE::"Posted FA Item Tracking Lines", TempFAItemLedgEntry);
            exit(true);
        end;
        exit(false);
    end;
