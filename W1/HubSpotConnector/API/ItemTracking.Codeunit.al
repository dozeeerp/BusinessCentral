...
codeunit 51303 "Item Tracking Lines mgt"
{
    var
    ...
    local procedure OnInsertRecord(BelowxRec: Boolean) Result: Boolean
    begin
    ...
    // local OnQueryClosePage(CloseAction: Action) Result: Boolean
    // var
    ...
    local procedure InsertRecord(var TempTrackingSpecification: Record "Tracking Specification" temporary)
    begin
    ...
    local procedure AddReservEntriesToTempRecSet(var ReservEntry: Record "Reservation Entry"; var TempTrackingSpecification: Record "Tracking Specification" temporary; SwapSign: Boolean; Color: Integer; SrcQtyRoundingPrecision: Decimal)
    var
    ...
    procedure InsertItemTracking(SerialNo: Code[50]; LotNo: Code[50]; Quantity: Decimal)
    var
        LotNo2: Code[50];
        IsHandled: Boolean;
    begin
        ...
        Rec.TransferFields(SourceTrackingSpecification);
        OnNewRecord();
        Rec.Validate("Quantity (Base)", 0);
        Rec.Validate("Qty. to Handle (Base)", 0);
        if SerialNo <> '' then begin
            rec.Validate("Serial No.", SerialNo);
            SerialNoOnAfterValidate();
            if Rec."Serial No." <> '' then begin
                IsHandled := false;
                OnValidateSerialNoOnBeforeFindLotNo(Rec, IsHandled);
                if not IsHandled then begin
                    ItemTrackingDataCollection.FindLotNoBySNSilent(LotNo2, Rec);
                    Rec.Validate("Lot No.", LotNo2);
                end;
            end;
        end;
        if (LotNo <> '') and (rec."Lot No." <> LotNo) then begin
            Rec.Validate("Lot No.", LotNo);
            LotNoOnAfterValidate();
        end;
        if Quantity <> 0 then begin
            Rec.Validate("Quantity (Base)", Quantity);
            QuantityBaseOnValidate();
            // QuantityBaseOnAfterValidate();
        end;
        if Rec."Qty. to Handle (Base)" <> Quantity then
            Rec.Validate("Qty. to Handle (Base)", Quantity);
