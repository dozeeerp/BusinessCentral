page 52102 "License Request"
{
    ...
    actions
    {
        ...
        area(Processing)
        {
            ...
            action(Activate)
            {
                ...
                trigger OnAction()
                var
                    LicenseActivate: Codeunit "License Mgmt.";
                    LiceReq_lRec: Record "License Request";
                    ActiveReqCardLicense_lPge: Page "Active License Card";
                    Code_lCod: Code[20];
                begin
                    Code_lCod := Rec."No.";
                    CheckValidations();
                    if LicenseActivate.Run(Rec) then begin
                        CurrPage.Close();
                        IF Confirm('Do you wish to Open Active License Card?', true) then begin
                            LiceReq_lRec.Reset();
                            LiceReq_lRec.SetRange("No.", Code_lCod);
                            ActiveReqCardLicense_lPge.SetTableView(LiceReq_lRec);
                            ActiveReqCardLicense_lPge.RunModal();
                        end;
                    end;
                end;
            }
    ...
    begin
        case Rec."License Type" of
            Rec."License Type"::Commercial:
                begin
                    IF Rec."Pre-paid" then begin
                        Rec.TestField("Invoice No");
                        CLE_lRec.Reset();
                        CLE_lRec.SetRange("Document No.", Rec."Invoice No");
                        CLE_lRec.SetRange("Document Type", CLE_lRec."Document Type"::Invoice);
                        IF CLE_lRec.FindFirst() then begin
                            CLE_lRec.CalcFields("Remaining Amount", "Remaining Amt. (LCY)", CLE_lRec."Amount (LCY)", CLE_lRec.Amount);
                            IF CLE_lRec."Remaining Amt. (LCY)" = CLE_lRec."Amount (LCY)" then Error('No Payment received against Invoice: %1.', Rec."Invoice No");
                        end;
                    end;
                end;
        // Rec."License Type"::MillionICU:
        //     begin
        //         Rec.TESTField("Doner ID");
        //     end;
        end;
    end;
}