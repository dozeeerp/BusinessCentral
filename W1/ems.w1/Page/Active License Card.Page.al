    ...
    actions
    {
        ...
        area(Processing)
        {
            ...
            action("Issue Add On")
            {
                ...
                begin
                    ...
                    if Rec."License Type" = Rec."License Type"::Commercial then begin
                        LicenseMgmt.CreateLicenseRequest(Rec."No.", Action_gOpt::"Issue Add On");
                    end;
                end;
            }
            action("Terminate")
            ...
            action("Update Document")
            {
                ...
                ToolTip = 'Add new information relevant to the document, such as change contact, KAM, Sales Persone.';

                trigger OnAction()
                var
                    ActiveLicenseUpdate: Page "Active License - Update";
                begin
                    if Rec.Status <> Rec.Status::Active then
                        Error('Update document is possible only for Active licenses.');
                    ActiveLicenseUpdate.LookupMode := true;
                    ActiveLicenseUpdate.SetRec(Rec);
                    ActiveLicenseUpdate.RunModal();
                end;
            }
        }
        area(Promoted)
        {
            group(Category_Process)
            {
                ...
                actionref(IssueExtensio_Promoted; "Issue Extension")
                {
                }
                actionref(IssueAddOn_Promoted; "Issue Add On")
                {
                }
                actionref(IssueRenewal_Promoted; "Issue Renewal")
                {
                }
                actionref(terminate_Promoted; Terminate)
                {
                }
                actionref(Devices_Promoted; Devices)
                {
                }
            }
            group(Category_Category4)
            {
                Caption = 'License', Comment = 'Generated from the PromotedActionCategories property index 3.';
